...
if(!subscription_schema_ready($pdo)||!subscription_self_service_schema_ready($pdo))redirect(url('/upgrade.php'));
...
function subscription_ui_plan_features(array $package,int $limit=6): array
{
    ...
    foreach(($package['entitlements']??[]) as $row){
        ...
        $catalog=subscription_capability_catalog();$meta=$catalog[$key]??null;if(!$meta)continue;
