...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        if($action==='update_finding'){
            vp3_research_update_finding_v2060($pdo,$uid,$projectId,trim((string)($_POST['finding_id']??'')),(string)($_POST['title']??''),(string)($_POST['body']??''));
            flash('research_project_notice','Finding updated.');research_project_redirect_v2060($projectId,'findings');
        }
...
<?php if($canResearch&&(string)$finding['status']!=='published'): ?><details class="finding-edit"><summary>Edit Finding</summary><form method="post"><?=csrf_field()?><input type="hidden" name="project_id" value="<?=research_p_e($projectId)?>"><input type="hidden" name="action" value="update_finding"><input type="hidden" name="finding_id" value="<?=research_p_e((string)$finding['id'])?>"><label class="field"><span>Title</span><input name="title" maxlength="190" required value="<?=research_p_e((string)$finding['title'])?>"></label><label class="field"><span>Statement / analysis</span><textarea name="body" rows="5"><?=research_p_e((string)$finding['body'])?></textarea></label><button type="submit">Save Finding</button></form></details><div class="actions"><form method="post"><?=csrf_field()?><input type="hidden" name="project_id" value="<?=research_p_e($projectId)?>"><input type="hidden" name="action" value="finding_status"><input type="hidden" name="finding_id" value="<?=research_p_e((string)$finding['id'])?>"><input type="hidden" name="status" value="<?=(string)$finding['status']==='confirmed'?'draft':'confirmed'?>"><button type="submit"><?=(string)$finding['status']==='confirmed'?'Return to draft':'Confirm Finding'?></button></form></div>
