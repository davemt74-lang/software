...
The package also includes `VP3_RELEASE.json`, generated by CI with the exact Git commit SHA, repository and workflow run ID so a deployed artifact can be traced to its green source tree.

For pull requests, both the v23.60 release gate and Production Deploy Package explicitly check out `github.event.pull_request.head.sha` rather than GitHub's synthetic merge ref. The embedded commit therefore identifies the exact tested feature tree.
