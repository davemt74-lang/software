...
- `homeserver_only` — use HomeServer and never fall back to VP3 Cloud.
...
4. never fall back from `homeserver_only`;
...
- `vp3_cloud` — HomeServer itself routed through VP3 Cloud under the paired wrapper policy.
...
3. HomeServer-only requests never execute or fall back through VP3 Cloud.
4. Automatic routing prefers HomeServer and falls back only when cloud capacity and HomeServer policy allow it.
