...
<section class="turns"><?php $shown=0;foreach((array)($pageData['segments']??[]) as $segment):if((string)($segment['segment_type']??'')!=='transcript')continue;$shown++;?><article class="turn"><header><strong><?= e((string)($segment['speaker_label']??'Speaker')) ?></strong><span><?= e(gmdate('i:s',max(0,(int)($segment['started_ms']??0))/1000)) ?></span></header><p><?= e((string)($segment['transcript_text']??'')) ?></p></article><?php endforeach;?><?php if(!$shown):?><div class="empty">No transcript text is stored on this evidence page.</div><?php endif;?></section>
</main></body></html>