...
<header class="chat-topbar personal-knowledge-topbar"><button class="chat-icon-button mobile-only" id="openChatSidebar" type="button" aria-label="Open menu">☰</button><div class="chat-topbar-title"><strong>My Knowledge</strong><span>Private, owner-scoped memory for your Agent</span></div><div class="chat-topbar-actions personal-knowledge-top-actions"><?php if($profileUrl!==''):?><a href="<?= e($profileUrl) ?>">View Profile</a><?php endif;?><?php if($profileAgentAllowed):?><a href="<?= e(url('/profile-agent.php')) ?>">Profile Agent</a><?php endif;?><?php if($canManage):?><a class="primary" href="<?= e(url('/knowledge.php#knowledge-form')) ?>">+ Add Knowledge</a><?php endif;?></div></header>
