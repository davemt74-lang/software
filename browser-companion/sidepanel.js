...
'openSearchPageBtn','searchInput','searchSubmitBtn','searchType','searchThisSource','searchContextText','saveSearchBtn','refreshDiscoveryBtn','searchResultHeading','searchResultCount','searchResultsEmpty','searchResultsList','savedSearchesEmpty','savedSearchesList','recentSearchesEmpty','recentSearchesList','clearRecentSearchesBtn','claimDialog','closeClaimDialogBtn','claimStatement','claimRationale','claimVisibility','claimTeamField','claimTeamSelect','claimSubmitBtn','notice'
].forEach(id=>ui[id]=$(id));
...
function searchParams(extra){
  const p={q:String(ui.searchInput.value||'').trim(),type:ui.searchType.value||'',context_source_id:currentSource&&currentSource.id||'',context_domain:host(capture&&capture.source_url||''),context_only:ui.searchThisSource.checked?'1':''};
