...
must(outcome.includes("'9999-12-31 23:59:59'"),'unresolved uncertain dispatch must pin duplicate protection');
must(outcome.includes('ON DUPLICATE KEY UPDATE intent_public_id=VALUES(intent_public_id),expires_at=VALUES(expires_at)'),'unresolved guard must be recreated if the original bounded guard expired');
