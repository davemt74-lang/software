...
const checks=[
 ...
 ['Cloud snapshot includes Brain knowledge contacts tasks calendar files notifications',["memory","knowledge","contacts","tasks","calendar","files","notifications"].every(x=>shared.includes("'"+x+"'"))],
 ...
 ['HomeServer datasets participate in central Agent data policy',/homeserver_memory/.test(catalog)&&/homeserver_knowledge/.test(catalog)&&/homeserver_contacts/.test(catalog)&&/homeserver_tasks/.test(catalog)&&/homeserver_calendar/.test(catalog)&&/homeserver_files/.test(catalog)&&/homeserver_notifications/.test(catalog)],
 ['Profile Agent uses same fabric only through existing policy gate',/homeserver_shared_v210_exchange/.test(profile)&&/user_data_policy_can_use_v236/.test(profile)&&/homeserver_memory/.test(profile)&&/homeserver_knowledge/.test(profile)&&/homeserver_contacts/.test(profile)&&/homeserver_tasks/.test(profile)&&/homeserver_calendar/.test(profile)&&/homeserver_files/.test(profile)&&/homeserver_notifications/.test(profile)],
 ['Profile Agent HomeServer resources remain denied by default',/profile_agent_allowed'=>false/.test(read('includes/user-agent-system-v236.php'))],
