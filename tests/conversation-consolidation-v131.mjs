...
assert(chatStream.includes('chat_v121_context($query,$user,$principal,$agentContext,$conversationId)')&&chatStream.includes('ai_v121_stream_chat_response($query,$history,$context,$user'),'stream Chat passes enriched v131 context and selected agent principal into the active NDJSON generation path');
...
assert(browserContext.includes("const conversationKey=`stonefellow:conversation-id:${userId}:${selectedUserAgentId||'system'}`")&&browserContext.includes('readStoredConversation')&&browserContext.includes('writeStoredConversation'),'shared Agent Context persists conversation identity per user and selected agent');
