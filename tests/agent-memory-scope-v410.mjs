...
assert.match(scope, /SET user_agent_id=NULL,[\s\S]*SHA1\(CONCAT\('v410\|agent:0\|',memory_hash\)\)/);
...
assert.match(lifecycle, /function agent_memory_v123_write_row\(int \$id,int \$uid,int \$agentId/);
...
assert.match(lifecycle, /SELECT \* FROM agent_memory_items WHERE user_id=\? AND '\.\$scope/);
...
// Same legacy memory identity must produce different durable hashes per Agent.
