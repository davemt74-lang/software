...
assert.match(service, /\$billingAlias\."\.status='active'"/);
assert.match(service, /\$localAlias\."\.status='active'"/);
assert.match(service, /billing_required=1/);
...
assert.match(service, /ROUND\(SUM\(CASE/);
assert.match(service, /p\.is_trial=1/);
assert.match(service, /MIN\(s\.starts_at\) first_trial_at/);
assert.match(service, /billing_subscriptions bs ON bs\.user_id=t\.user_id/);
assert.match(service, /bs\.status NOT IN \('trialing','incomplete','incomplete_expired'\)/);
assert.match(service, /bs\.created_at>=t\.first_trial_at/);
assert.match(service, /COALESCE\(s\.ai_token_override,p\.trial_tokens,0\) token_allowance/);
