...
assert.match(scope,/'effective_preference'\]\s*=\s*'homeserver_only'/,'cloud-disabled scope must resolve to HomeServer-only');
assert.match(scope,/source_before_scope/,'scope narrowing must preserve legacy compute-policy source');
assert.match(scope,/last_known_cloud_blocked/,'scope resolver must retain a last-known restrictive cloud boundary');
