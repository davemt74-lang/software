...
assert.match(runtime,/information_schema\.tables/,'public release lookup must use a deterministic MySQL\/MariaDB table probe');
...
assert.match(runtime,/VP3 Browser Companion/,'manager must reject packages for a different extension');
assert.match(runtime,/256 MB safety limit/,'manager must bound total uncompressed ZIP size');
