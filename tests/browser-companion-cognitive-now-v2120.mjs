...
must(js.includes("['now','agent'].includes(activeView)&&!c.has('agent.message')")||js.includes("activeView==='now'&&!c.has('agent.message')"),
  'Now must fall back when Agent access is unavailable');
