...
assert.ok(service.includes('idempotent')&&service.includes("hash_equals((string)$prior['plan_snapshot_hash'],$snapshotHash)")&&service.includes('video_meeting_plan_action_bound_draft_hash_v18190($existing)'),'same-plan and same-plan-bound-draft handoff must be idempotent');
...
assert.ok(service.includes("'non_plan_action_fields_editable'=>true"),'recipient/lead/title and other non-plan fields must remain editable without plan drift');
...
assert.ok(ui.includes('Plan-bound action fields changed after handoff'),'UI must distinguish action-draft drift from plan drift');
assert.ok(ui.includes('non-plan fields can still be edited without causing plan drift'),'UI must explain allowed action-specific edits');
