...
assert.ok(!service.includes('recipient_email\']=')&&!service.includes('lead_id\']='),'handoff must not infer CRM or email recipients');
assert.ok(service.includes("'explicit_handoff'=>true")&&service.includes("'auto_approval'=>false")&&service.includes("'auto_execution'=>false")&&service.includes("'recipient_inference'=>false"),'handoff policy boundary missing');
...
for(const action of ['approve','execute','retry'])assert.ok(actionsApi.includes(action),`missing guarded action ${action}`);
...
assert.ok(ui.includes('Approval and execution are blocked'),'stale warning missing');
