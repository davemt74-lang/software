...
assert.ok(guard.includes('video_meeting_continuity_related_thread_v18210'),'descendant continuity lineage must be checked');
assert.ok(guard.includes("l.status IN ('carried_forward','resolved_elsewhere')"),'active/resolved descendant links must be visible to the guard');
...
assert.ok(guard.includes("(int)($related['execution_id']??0)>0"),'descendant action history must block duplicate carry');
assert.ok(guard.includes("continuity_status']??'')==='carried_forward'"),'an already-active descendant thread must block a second active carry');
...
assert.ok(prepApi.includes("$prep['continuity']=")&&prepApi.includes('video_meeting_continuity_guarded_state_v18210'),'Prep must surface continuity state');
assert.ok(core.includes("'agent_context'=>"),'continuity must expose bounded Agent context');
...
for(const writer of ['user_calendar_automation_create_event_v1300','crm_v180_activity(','agent_workflow_create_from_priority_v1400','agent_appointment_lifecycle_email_v700(','mail(']){
  assert.ok(!core.includes(writer)&&!guard.includes(writer),`continuity must not execute downstream writer ${writer}`);
}
