...
assert.doesNotMatch(music, /INSERT\s+INTO\s+user_account_types/i);
assert.doesNotMatch(music, /UPDATE\s+users\s+SET\s+role/i);

const musicPermissions = music.match(/function music_workspace_owner_permissions_v320\(\): array[\s\S]*?\n}/)?.[0] || '';
for (const permission of ['tracks.manage','albums.manage','release.manage','producer.access','team.manage']) assert.match(musicPermissions, new RegExp(permission.replace('.', '\\.')));
for (const forbidden of ['admin.access','messages.manage','users.manage','ai.manage','permissions.manage']) assert.doesNotMatch(musicPermissions, new RegExp(`['\"]${forbidden.replace('.', '\\.')}['\"]`));
assert.match(music, /preserves_data_on_disable'=>true/);
assert.doesNotMatch(plugins, /DELETE\s+FROM\s+(?:tracks|albums|artist_|human_|users)/i);

for (const table of ['user_follows','user_friendships','user_blocks','human_conversations','human_conversation_members','human_messages','human_message_requests']) assert.match(social, new RegExp(table));
assert.doesNotMatch(social, /profile_agent_messages|chat_messages/);
