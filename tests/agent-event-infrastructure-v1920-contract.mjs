...
for(const field of ['event_uuid','owner_user_id','source','event_type','dedupe_hash','verification_status','processing_status','payload_json','linked_run_id','replay_count']){
...
// Event ingress is durable, bounded, sanitized and owner-scoped.
...
assert.ok(core.includes('UNIQUE KEY uq_agent_event_owner_dedupe')||migration.includes('UNIQUE KEY uq_agent_event_owner_dedupe'));
assert.ok(core.includes("verification_status']??'trusted'"));
assert.ok(core.includes("['trusted','verified']"));
assert.ok(!core.includes("verification_status']??'unverified'"));
...
// Public webhook route cannot register sources or execute tools.
...
assert.ok(!webhook.includes("$e->getMessage()"));
...
// Replay re-dispatches the immutable sanitized event and deterministic workflow identity.
...
assert.ok(api.includes("'action']??'')!=='replay'"));
...
// Owner page is observability + safe replay only.
