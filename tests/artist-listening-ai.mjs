...
assert.ok(page.includes(asset), 'Artist Listening must load the canonical v304 AI controller exactly once');
...
assert.ok(client.includes("const BUILD = 'transcription-workflow-v304-20260906'"), 'controller must identify the v304 workflow build');
...
/* Canonical registries remain intact while v302 items, v303 actions and v304 workflow extend them. */
...
/* Stable v300 URL delegates through the v304 workflow while prior contracts stay available. */
...
assert.ok(api.includes('transcription_app_analyze_v304'), 'stable endpoint must execute through v304');
