...
assert.match(memberMenu, /data-vp3-global-agent-voice/, 'shared member menu must render the focused Agent Voice control inside the Agent + Voice dashboard');
assert.match(memberMenu, /My ElevenLabs voice clone/, 'shared member menu must expose the existing user clone setting rather than a second voice system');
assert.match(agentVoiceMenu, /save_agent_voice/, 'compact Agent Voice control must persist through canonical Chat Settings');
assert.match(agentVoiceMenu, /voice_enabled/, 'per-Agent voice selection must reuse the existing account voice_enabled setting');
assert.match(agentVoiceMenu, /has_clone_binding/, 'clone selection must be backed by the real Voice Profile binding');
assert.match(agentVoiceMenu, /stonefellow:agent-voice/, 'compact Agent Voice changes must be broadcast to active Chat runtime');
assert.doesNotMatch(agentVoiceMenu, /ELEVENLABS_API_KEY|xi-api-key|ai_elevenlabs_api_key|ai_decrypt_secret/i, 'browser Agent Voice controls must never receive ElevenLabs credentials');
assert.match(voiceProfileApi, /studio_voice_profile_elevenlabs_key/, 'ElevenLabs provider credentials must remain server-side in the existing Voice Profile implementation');
assert.match(memberNavigation, /data-agent-voice-toggle/, 'canonical navigation helper may retain the legacy Agent Voice toggle markup for backward-compatible surfaces');
assert.match(memberNavigation, /has_permission\('chat\.access',\s*\$user\)/, 'Agent Voice controls must remain Chat-permission gated');
assert.match(settingsJs, /ensureAgentVoiceToggle/, 'legacy Main Feed profile menu remains backward compatible with the established Agent Voice setting');
