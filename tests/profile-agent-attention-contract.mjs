...
assert.match(memberNav,/personal_capability_has_v242\('profile_agent\.access',\$user\)[\s\S]*Profile Agent[\s\S]*url\('\/profile-agent\.php'\)/,'canonical user navigation exposes Profile Agent only through its personal capability gate');
assert.match(sidebar,/require __DIR__ \. '\/main-sidebar\.php';/,'workspace sidebar must delegate navigation ownership to the canonical member sidebar');
assert.match(mainSidebar,/member_navigation_menu_links\(\$mainSidebarUser\)/,'canonical Agent sidebar must source secondary destinations from canonical user navigation');
assert.match(mainSidebar,/data-agent-user-footer/,'canonical Agent sidebar must place Profile Agent and other secondary destinations in the bottom user menu');
assert.doesNotMatch(mainSidebar,/account\.php#profile-agent/,'canonical member sidebar no longer points at the removed account section');
