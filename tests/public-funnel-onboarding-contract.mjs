...
for (const [name, source] of Object.entries({signup, login})) {
  ...
  assert.match(source, /\['plan','billing','source','return_to'\]/, `${name} form must preserve validated funnel fields across validation failures`);
...
assert.match(demo, /\['plan','billing','source','return_to'\]/, 'demo form must preserve validated funnel state across submission');
