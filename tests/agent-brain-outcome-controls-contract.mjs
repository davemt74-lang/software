...
/* The server, not browser-supplied metadata, authorizes which priority can be closed. */
...
assert.ok(api.includes("preg_match('/^notification:(\\d+)$/'"), 'notification priority hashes must be derived server-side');
assert.ok(api.includes("$source === 'analytics'"), 'Analytics priority hashes must be derived server-side');
assert.ok(api.includes("str_starts_with($key, 'task:')"), 'task priority hashes must be derived server-side');
assert.ok(api.includes("str_starts_with($key, 'ecosystem:')"), 'ecosystem priority hashes must be derived server-side');
...
assert.ok(api.includes('hash_equals((string)($candidate[\'outcome_hash\'] ?? \'\'), $hash)'), 'server must exact-match the submitted hash to a current priority');
