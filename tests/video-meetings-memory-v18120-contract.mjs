...
assert.ok(!p2.includes('raw_transcript'));
for(const forbidden of ['transcript_text','source_excerpt','relay_credentials','provider_secret','note_text','followup_body','follow_up_draft'])assert.ok(!memory.includes(forbidden),`private field leaked into memory module: ${forbidden}`);
