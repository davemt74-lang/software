...
function calendar_v1300_source_label(array $event): string{return match(calendar_v1300_source($event)){'booking'=>'Booking','agent'=>'Agent','automation'=>'Automated',default=>'Personal'};}
...
function calendar_v1300_event_url(array $event): string{return (string)$event['kind']==='booking'?url('/scheduling.php?schedule='.(int)($event['schedule_id']??0).'&tab=bookings'):url('/calendar-event.php?id='.(int)$event['id']);}
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#f7f7f5"><title><?= e(system_agent_name()) ?> | Calendar</title><link rel="stylesheet" href="<?= e(url('/chat.css?v=82')) ?>"><link rel="stylesheet" href="<?= e(url('/calendar-v1300.css?v=1300')) ?>"></head>
...
<?php $memberHeaderUser=$user;$memberHeaderTitle='Calendar';$memberHeaderSubtitle='Bookings, personal events + Agent-managed time';$memberHeaderActions='<a class="calendar-button" href="'.e(url('/scheduling.php')).'">Scheduling</a><a class="calendar-button primary" href="'.e(url('/calendar-event.php')).'">New event</a>';require __DIR__.'/includes/member-header.php'; ?>
...
<section class="calendar-toolbar"><div class="calendar-title"><span>User Calendar</span><h1>Your time, in one place.</h1><p>VP3 bookings appear here automatically. You and your Agent can add personal work, reminders and other dated events without changing the booking lifecycle.</p></div><div class="calendar-actions"><a class="calendar-button" href="<?= e(url('/chat.php')) ?>">Ask Agent</a><a class="calendar-button primary" href="<?= e(url('/calendar-event.php')) ?>">+ New event</a></div></section>
...
<?php foreach($agenda as $event): $source=calendar_v1300_source($event); ?><a class="calendar-agenda-row" href="<?= e(calendar_v1300_event_url($event)) ?>" style="color:inherit;text-decoration:none"><div><strong><?= e(calendar_v1300_agenda_time($event,$tz)) ?></strong><small><?= e($timezone) ?></small></div><div><strong><?= e((string)$event['title']) ?></strong><small><?= e((string)($event['description']??'')) ?></small></div><div><strong><?= e((string)($event['location']??'')) ?></strong><small><?= (string)$event['kind']==='booking'&&trim((string)($event['guest_name']??''))!==''?'With '.e((string)$event['guest_name']):'VP3 Calendar' ?></small></div><span class="calendar-source <?= e($source) ?>"><?= e(calendar_v1300_source_label($event)) ?></span></a><?php endforeach; ?>
