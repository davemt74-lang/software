...
function render(status,{preserveAlert=false}={}){status=status||{};const state=status.connection_state||'not_connected';
 ...
 else if(state==='connected'){els.title.textContent='HomeServer connected';els.detail.textContent='VP3 is receiving the live HomeServer HTTPS heartbeat and can use only the capabilities authorized for this pairing.';}
