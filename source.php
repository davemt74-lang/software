...
if(!$source){
    ...
    // visitors. A public Source page requires either an authorized annotation
    // or an authorized published Research report that cites this Source.
    if($userId<1 && empty($feed['items']) && empty($publishedResearch)){
        http_response_code(404);
        $source=null;
        $pageTitle='Source not found';
    }
