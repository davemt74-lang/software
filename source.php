...
if(!$source){
    ...
    if(function_exists('vp3_search_schema_ready_v2090')&&vp3_search_schema_ready_v2090($pdo)){
        try{
            $relatedPayload=vp3_search_query_v2090($pdo,$userId,'',['context_source_id'=>(string)$source['public_id'],'context_only'=>true],16,false);
            $relatedSearch=array_values(array_filter((array)($relatedPayload['items']??[]),static fn(array $item): bool=>(string)$item['type']!=='source'));
        }catch(Throwable $e){$relatedSearch=[];}
    }
...
<?php if(!empty($relatedSearch)): ?><section class="related-section"><div class="eyebrow">Related on VP3</div>
<?php foreach($relatedSearch as $related): ?><article class="related-card"><strong><?=source_page_e_v2050((string)$related['title'])?></strong><?php if(!empty($related['snippet'])): ?><p><?=source_page_e_v2050((string)$related['snippet'])?></p><?php endif; ?><div class="research-meta"><?=source_page_e_v2050(ucfirst((string)$related['type']))?><?php if(!empty($related['status'])): ?> · <?=source_page_e_v2050(str_replace('_',' ',(string)$related['status']))?><?php endif; ?></div><div class="actions"><a href="<?=source_page_e_v2050((string)$related['url'])?>">Open</a></div></article><?php endforeach; ?><div class="actions"><a href="<?=source_page_e_v2050(url('/search.php?source='.rawurlencode((string)$source['public_id']).'&context_only=1'))?>">Search this Source</a></div></section><?php endif; ?>
