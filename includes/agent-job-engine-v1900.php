...
function agent_job_control_ready_v1900(?PDO $pdo=null): bool
{
    $pdo??=db();if(!$pdo)return false;
    foreach(['work_priority','pause_requested_at','paused_at','paused_from_status'] as $c)if(!column_exists('agent_workflow_runs',$c))return false;
    return true;
}
function agent_job_pause_requested_v1900(array $run): bool{return trim((string)($run['pause_requested_at']??''))!=='';}
function agent_job_clear_control_state_v1900(PDO $pdo,int $uid,int $runId): void
{
    if(!agent_job_control_ready_v1900($pdo)||$uid<1||$runId<1)return;
...
function agent_job_public_run_v1900(PDO $pdo,array $row,bool $history=false): array
{
    ...
    if(agent_job_control_ready_v1900($pdo)){
        $out['work_priority']=max(1,min(100,(int)($row['work_priority']??50)));
...
function agent_job_record_result_v1900(PDO $pdo,array $user,int $runId,int $actionId,string $leaseToken,string $idempotencyKey,bool $success,string $summary,array $result=[],string $errorClass='',bool $retryable=true): array
{
    ...
    try{$pdo->beginTransaction();$e=$pdo->prepare('SELECT * FROM agent_workflow_receipts WHERE owner_user_id=? AND receipt_key=? LIMIT 1 FOR UPDATE');$e->execute([$uid,$receiptKey]);$receipt=$e->fetch();if(is_array($receipt)){$pdo->commit();$row=agent_workflow_row_v1400($pdo,$uid,$runId);return ['duplicate'=>true,'receipt_id'=>(int)$receipt['id'],'run'=>$row?agent_job_public_run_v1900($pdo,$row,true):null,'build'=>VP3_AGENT_JOB_ENGINE_V1900];}
        ...
        $control=agent_job_control_ready_v1900($pdo);$pauseAfterCurrent=$control&&agent_job_pause_requested_v1900($run);
        ...
        if($success){$pdo->prepare("UPDATE agent_workflow_actions SET status='completed',result_summary=?,result_json=?,error_class='',completed_at=UTC_TIMESTAMP(),heartbeat_at=UTC_TIMESTAMP(),progress_percent=100,progress_message='Completed' WHERE id=? AND owner_user_id=?")->execute([$summary,$safe?agent_workflow_json_v1400($safe):null,$actionId,$uid]);agent_workflow_event_v1400($pdo,$uid,$runId,'action_completed','executing','executing','executor',$summary!==''?$summary:'Workflow action completed.',['action_id'=>$actionId,'receipt_id'=>$receiptId]);$p=$pdo->prepare("SELECT COUNT(*) FROM agent_workflow_actions WHERE run_id=? AND owner_user_id=? AND status IN ('queued','approval_pending','executing')");$p->execute([$runId,$uid]);
            if((int)$p->fetchColumn()===0){$pdo->prepare("UPDATE agent_workflow_runs SET status='completed',".agent_job_release_sql_v1900().",last_error_class='',next_attempt_at=NULL,progress_percent=100,progress_message='Completed',completed_at=UTC_TIMESTAMP() WHERE id=? AND owner_user_id=?")->execute([$runId,$uid]);if($control)agent_job_clear_control_state_v1900($pdo,$uid,$runId);agent_workflow_event_v1400($pdo,$uid,$runId,'completed','executing','completed','executor','Durable workflow completed with receipt-backed execution.',['receipt_id'=>$receiptId]);$terminal='completed';}
            elseif($pauseAfterCurrent){$pdo->prepare("UPDATE agent_workflow_runs SET status='paused',".agent_job_release_sql_v1900().",last_error_class='',next_attempt_at=COALESCE(next_attempt_at,UTC_TIMESTAMP()),progress_percent=0,progress_message='Paused after current action',pause_requested_at=NULL,paused_at=UTC_TIMESTAMP(),paused_from_status='approved' WHERE id=? AND owner_user_id=?")->execute([$runId,$uid]);agent_workflow_event_v1400($pdo,$uid,$runId,'paused','executing','paused','system','Workflow paused safely after the active leased action completed.',['action_id'=>$actionId,'receipt_id'=>$receiptId]);}
...
function agent_job_recover_expired_v1900(PDO $pdo,array $user,int $limit=25): array
{
    ...
    foreach($s->fetchAll()?:[] as $item){$runId=(int)$item['id'];$terminal=false;try{$pdo->beginTransaction();$run=agent_workflow_row_v1400($pdo,$uid,$runId,true);if(!$run||(string)$run['status']!=='executing'||(strtotime((string)($run['lease_expires_at']??''))?:0)>=time()){$pdo->rollBack();continue;}$control=agent_job_control_ready_v1900($pdo);$pauseAfterCurrent=$control&&agent_job_pause_requested_v1900($run);$actionId=(int)($run['current_action_id']??0);$q=$pdo->prepare('SELECT * FROM agent_workflow_actions WHERE id=? AND run_id=? AND owner_user_id=? LIMIT 1 FOR UPDATE');$q->execute([$actionId,$runId,$uid]);$action=$q->fetch();if(!$action){$pdo->rollBack();continue;}$attempt=(int)($action['attempt_count']??1);$max=max(1,(int)($action['max_attempts']??$run['max_attempts']??3));
        if($attempt<$max){$when=gmdate('Y-m-d H:i:s',time()+agent_job_retry_delay_v1900($attempt,(int)($run['retry_backoff_seconds']??60)));$pdo->prepare("UPDATE agent_workflow_actions SET status='queued',available_at=?,heartbeat_at=NULL,error_class='lease_expired',progress_percent=0,progress_message='Recovered after expired lease' WHERE id=? AND owner_user_id=?")->execute([$when,$actionId,$uid]);
            if($pauseAfterCurrent){$pdo->prepare("UPDATE agent_workflow_runs SET status='paused',".agent_job_release_sql_v1900().",last_error_class='lease_expired',next_attempt_at=?,progress_percent=0,progress_message='Paused · recovered retry waiting',pause_requested_at=NULL,paused_at=UTC_TIMESTAMP(),paused_from_status='approved' WHERE id=? AND owner_user_id=?")->execute([$when,$runId,$uid]);agent_workflow_event_v1400($pdo,$uid,$runId,'paused','executing','paused','system','Expired lease recovered and the requested pause was honored before retry.',['action_id'=>$actionId,'retry_at'=>$when]);}
        ...
        else{$pdo->prepare("UPDATE agent_workflow_actions SET status='failed',error_class='lease_expired',completed_at=UTC_TIMESTAMP(),progress_message='Lease expired' WHERE id=? AND owner_user_id=?")->execute([$actionId,$uid]);$pdo->prepare("UPDATE agent_workflow_runs SET status='failed',".agent_job_release_sql_v1900().",last_error_class='lease_expired',next_attempt_at=NULL,progress_message='Lease expired' WHERE id=? AND owner_user_id=?")->execute([$runId,$uid]);if($control)agent_job_clear_control_state_v1900($pdo,$uid,$runId);agent_workflow_event_v1400($pdo,$uid,$runId,'failed','executing','failed','system','Execution lease expired after the maximum attempt count.',['action_id'=>$actionId]);$failed++;$terminal=true;}$pdo->commit();$row=agent_workflow_row_v1400($pdo,$uid,$runId);if($row){agent_job_brain_memory_v1900($user,$row,$terminal?'failed':'recovered');if($terminal)agent_job_brain_outcome_v1900($user,$row,'failed');}}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();}}
