 ...
 *
 * Conversational, owner-scoped scheduling skills for Agent Chat plus a safe
 * read-only Profile Agent scheduling skill. Calendar mutations are prepared
 * first and require an explicit second-turn confirmation from the same user,
 * conversation and Agent principal before execution.
 */
...
/**
 * Resolve an existing active schedule without creating anything. Named Agents
 * prefer their bound schedule; the system Agent and unbound Agents use the
 * account default active schedule. Read-only Agent questions must not mutate DB.
 */
function agent_scheduling_tools_schedule_v460(PDO $pdo,array $user,int $agentId=0): ?array
{
    $userId=(int)($user['id']??0);
    if($userId<1||!agent_scheduling_tools_ready_v460($pdo))return null;
    try{
        if($agentId>0){
            $stmt=$pdo->prepare('SELECT * FROM agent_scheduling_schedules WHERE owner_user_id=? AND agent_id=? AND is_active=1 ORDER BY is_default DESC,id ASC LIMIT 1');
            $stmt->execute([$userId,$agentId]);
            $bound=$stmt->fetch();
            if($bound)return $bound;
        }
        $stmt=$pdo->prepare('SELECT * FROM agent_scheduling_schedules WHERE owner_user_id=? AND is_active=1 ORDER BY is_default DESC,id ASC LIMIT 1');
        $stmt->execute([$userId]);
...
{
    if(!$schedule||(int)$schedule['owner_user_id']!==(int)($user['id']??0)||empty($schedule['is_active']))return [];
    try{
...
function agent_scheduling_tools_date_v460(string $query,string $timezone,?string $fallbackDate=null): ?string
{
    ...
    if(preg_match('/\b(?:(next|this)\s+)?(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/i',$query,$m)){
        $modifier=trim((string)($m[1]??''));$weekday=strtolower($m[2]);
...
function agent_scheduling_tools_time_request_v460(string $query): ?array
{
    ...
    if(preg_match('/\b(?:at|around)\s+(\d{1,2})(?:\s*o\'?clock)?\b/i',$query,$m)){
        $hour=(int)$m[1];if($hour<1||$hour>12)return null;
...
function agent_scheduling_tools_slots_excluding_v460(PDO $pdo,array $event,string $date,int $excludeBookingId=0): array
{
    ...
    $duration=max(5,(int)$event['duration_minutes']);$interval=max(5,(int)$event['slot_interval_minutes']);$slots=[];
    foreach(agent_scheduling_windows_for_date_v430($pdo,$event,$date) as [$windowStart,$windowEnd]){
        for($minute=$windowStart;$minute+$duration<=$windowEnd;$minute+=$interval){
            $local=$day->setTime(intdiv($minute,60),$minute%60);$utc=$local->setTimezone(new DateTimeZone('UTC'));
            try{agent_scheduling_validate_start_v430($pdo,$event,$utc,$excludeBookingId);}catch(RuntimeException $e){continue;}
            $slots[]=['start_at_utc'=>$utc->format('Y-m-d H:i:s'),'end_at_utc'=>$utc->modify('+'.$duration.' minutes')->format('Y-m-d H:i:s'),'start_local'=>$local->format('Y-m-d H:i:s'),'timezone'=>$timezone];
        }
    }
    return $slots;
}
...
{
    if(!$slots||!$time)return ['slot'=>null,'ambiguous'=>false,'matches'=>[]];
    $matches=[];
    foreach($slots as $slot){
        try{$local=new DateTimeImmutable((string)$slot['start_local'],new DateTimeZone((string)$slot['timezone']));}catch(Throwable $e){continue;}
        $h=(int)$local->format('G');$m=(int)$local->format('i');
        if(!empty($time['ambiguous'])){
            if(($h%12)===((int)$time['hour']%12)&&$m===(int)$time['minute'])$matches[]=$slot;
        }elseif($h===(int)$time['hour']&&$m===(int)$time['minute'])$matches[]=$slot;
    }
...
function agent_scheduling_tools_guest_name_v460(string $query): string
{
    $patterns=[
        ...
        '/\b(?:book|schedule)\s+([\pL][\pL .\'\-]{0,70}?)(?=\s+(?:on|at|tomorrow|today|next|this)\b|[,;.!?]|$)/iu',
    ];
    foreach($patterns as $pattern){
        if(!preg_match($pattern,$query,$m))continue;
        $name=trim(preg_replace('/\s+/u',' ',$m[1])??'');
        if($name===''||preg_match('/^(?:me|myself|you|the agent|a client|a meeting|an appointment|a call|a session)$/i',$name))continue;
        return mb_strimwidth($name,0,190,'');
    }
    return '';
}

function agent_scheduling_tools_booking_rows_v460(PDO $pdo,array $user,int $scheduleId,int $limit=30,bool $upcomingOnly=true): array
{
    $userId=(int)($user['id']??0);if($userId<1||$scheduleId<1)return [];
    ...
    try{
        $stmt=$pdo->prepare(
            "SELECT b.*,e.slug AS event_slug,s.name AS schedule_name
             ...
             WHERE b.owner_user_id=? AND b.schedule_id=? {$where}
             ORDER BY b.start_at_utc ".($upcomingOnly?'ASC':'DESC').",b.id ".($upcomingOnly?'ASC':'DESC')."
             LIMIT ".max(1,min(100,$limit))
        );
        $stmt->execute([$userId,$scheduleId]);return $stmt->fetchAll()?:[];
    }catch(Throwable $e){return [];}
}

function agent_scheduling_tools_find_booking_v460(PDO $pdo,array $user,int $scheduleId,string $query): array
{
    $rows=agent_scheduling_tools_booking_rows_v460($pdo,$user,$scheduleId,60,true);
    if(!$rows)return ['booking'=>null,'matches'=>[]];
    if(preg_match('/(?:booking|appointment)\s*#?\s*(\d{1,10})\b/i',$query,$m)){
        $id=(int)$m[1];foreach($rows as $row)if((int)$row['id']===$id)return ['booking'=>$row,'matches'=>[$row]];
    }
    $q=mb_strtolower($query);$dateHint=agent_scheduling_tools_date_v460($query,(string)$rows[0]['organizer_timezone']);$scored=[];
    foreach($rows as $row){
        $score=0;
        foreach(['guest_name'=>30,'guest_email'=>45,'event_title'=>18] as $field=>$weight){
            ...
            foreach(preg_split('/[^\pL\pN@._-]+/u',$value)?:[] as $term)if(mb_strlen($term)>=3&&str_contains($q,$term))$score+=4;
        }
        if($dateHint!==null){
            $local=agent_scheduling_tools_slot_label_v460((string)$row['start_at_utc'],(string)$row['organizer_timezone'],'Y-m-d');
            $score+=$local===$dateHint?35:-5;
        }
...
{
    $key=agent_scheduling_tools_pending_key_v460((int)($user['id']??0),$conversationId);$pending=$_SESSION[$key]??null;
    if(!is_array($pending))return null;
    if((int)($pending['expires_at']??0)<time()){unset($_SESSION[$key]);return null;}
    if((int)($pending['user_id']??0)!==(int)($user['id']??0)||(int)($pending['conversation_id']??0)!==$conversationId){unset($_SESSION[$key]);return null;}
    return $pending;
}
...
{
    if($conversationId<1)throw new RuntimeException('A live Agent Chat conversation is required before changing your calendar.');
    ...
    $pending=[
        ...
        'risk'=>$risk,'created_at'=>time(),'expires_at'=>time()+900,'nonce'=>bin2hex(random_bytes(16)),
    ];
...
{
    $operation=(string)$pending['operation'];$payload=(array)($pending['payload']??[]);$agentId=max(0,(int)($pending['agent_id']??0));
    $currentAgentId=agent_scheduling_tools_conversation_agent_id_v460($pdo,$user,$conversationId);
    if($currentAgentId!==$agentId)throw new RuntimeException('The Agent principal changed before approval. Please prepare the scheduling action again.');
    ...
    if($operation==='reschedule'){
        ...
        $stmt->execute([(int)($payload['booking_id']??0),(int)$user['id'],(int)($payload['schedule_id']??0)]);$booking=$stmt->fetch();
    ...
    if($operation==='cancel'){
        $bookingId=(int)($payload['booking_id']??0);$scheduleId=(int)($payload['schedule_id']??0);
        $stmt=$pdo->prepare('SELECT * FROM agent_scheduling_bookings WHERE id=? AND owner_user_id=? AND schedule_id=? LIMIT 1');
...
{
    $lines=[];foreach(array_slice($rows,0,5) as $row)$lines[]='• #'.(int)$row['id'].' · '.(string)$row['guest_name'].' · '.(string)$row['event_title'].' · '.agent_scheduling_tools_slot_label_v460((string)$row['start_at_utc'],$timezone);
    return implode("\n",$lines);
}
...
function agent_scheduling_tools_intent_v460(string $query,array $events=[]): string
{
    ...
    if($music)return '';
    $scheduling=(bool)preg_match('/\b(?:calendar|calender|appointment|appointments|meeting|meetings|availability|available|free time|open time|open slot|time slot|schedule|scheduling|reschedule|re-schedule|cancellation)\b/u',$q);
    if(!$scheduling&&preg_match('/\bbook\b/u',$q))$scheduling=(bool)preg_match('/\b(?:today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday|at\s+\d|with\s+[\pL]|for\s+\d+\s*(?:min|hour))\b/u',$q);
    if(!$scheduling){foreach($events as $event){$title=mb_strtolower(trim((string)($event['title']??'')));if($title!==''&&str_contains($q,$title)){$scheduling=true;break;}}}
    if(!$scheduling)return '';
    if(preg_match('/\b(?:cancel|delete|remove)\b.*\b(?:appointment|meeting|booking)\b|\b(?:appointment|meeting|booking)\b.*\b(?:cancel|delete|remove)\b|\bconfirm cancellation\b/u',$q))return 'cancel';
...
function agent_scheduling_tools_query_v460(string $query,array $user,int $conversationId=0): array
{
    ...
    if($pending&&agent_scheduling_tools_reject_confirmation_v460($query)){
        ...
        agent_tool_log($user,'scheduling.approval',$query,'denied',['operation'=>(string)$pending['operation']],$conversationId);return $result;
    }
    if($pending&&agent_scheduling_tools_confirmation_v460($query)){
        try{
            $result=agent_scheduling_tools_execute_pending_v460($pdo,$user,$conversationId,$pending);agent_scheduling_tools_clear_pending_v460($user,$conversationId);
            $result['approval']=['required'=>true,'approved'=>true,'risk'=>$pending['risk']??[],'version'=>'v4.60'];return $result;
        }catch(Throwable $e){
            agent_scheduling_tools_clear_pending_v460($user,$conversationId);$result=$empty;$result['handled']=true;
            $result['answer']='I could not complete that scheduling change: '.$e->getMessage();
            agent_tool_log($user,'scheduling.'.(string)$pending['operation'],$query,'failed',['error'=>$e->getMessage()],$conversationId);return $result;
        }
    }
    if(!$pending&&agent_scheduling_tools_confirmation_v460($query)){
        $result=$empty;$result['handled']=true;$result['answer']='There is no pending scheduling change to confirm. Tell me what you want to book, reschedule, or cancel first.';return $result;
    }

    $agentId=agent_scheduling_tools_conversation_agent_id_v460($pdo,$user,$conversationId);
    $schedule=agent_scheduling_tools_schedule_v460($pdo,$user,$agentId);
    $events=$schedule?agent_scheduling_tools_events_v460($pdo,$user,$schedule):[];
    $intent=agent_scheduling_tools_intent_v460($query,$events);
    if($intent==='')return $empty;
    if(!$schedule){
        $result=$empty;$result['handled']=true;$result['answer']='Scheduling is not set up for this Agent yet. Open Scheduling to add an active schedule, availability and appointment type.';
        $result['actions'][]=['type'=>'open_url','label'=>'Open Scheduling','url'=>url('/scheduling.php')];return $result;
    }

    $scheduleId=(int)$schedule['id'];$timezone=agent_scheduling_timezone_v430((string)$schedule['timezone']);$result=$empty;$result['handled']=true;
    $result['actions'][]=['type'=>'open_url','label'=>'Open Scheduling','url'=>url('/scheduling.php?schedule='.$scheduleId)];

    if($intent==='list'){
        $rows=agent_scheduling_tools_booking_rows_v460($pdo,$user,$scheduleId,20,true);
        $date=agent_scheduling_tools_date_v460($query,$timezone);
        if($date!==null)$rows=array_values(array_filter($rows,fn(array $row):bool=>agent_scheduling_tools_slot_label_v460((string)$row['start_at_utc'],$timezone,'Y-m-d')===$date));
        if(!$rows)$result['answer']=$date!==null?'You have no active appointments on '.$date.'.':'Your calendar is clear — there are no upcoming confirmed or pending appointments on this schedule.';
        else{
            $lines=[];foreach(array_slice($rows,0,12) as $row)$lines[]='• #'.(int)$row['id'].' · '.agent_scheduling_tools_slot_label_v460((string)$row['start_at_utc'],$timezone).' · '.(string)$row['event_title'].' with '.(string)$row['guest_name'];
            $result['answer']="Here are your upcoming appointments in {$timezone}:\n".implode("\n",$lines);
        }
        agent_tool_log($user,'scheduling.list',$query,'success',['count'=>count($rows),'schedule_id'=>$scheduleId,'agent_id'=>$agentId],$conversationId);return $result;
    }
    ...
    if($intent==='availability'){
        ...
        if(!$event){
            ...
            $result['answer']=$choices?"Which appointment type should I check?\n".implode("\n",$choices):'This schedule has no active appointment type yet. Open Scheduling to create one.';return $result;
        }
        $date=agent_scheduling_tools_date_v460($query,$timezone);$days=[];
        ...
        for($i=0;$i<$scanDays&&count($days)<5;$i++){
            $d=$start->modify('+'.$i.' days')->format('Y-m-d');$slots=agent_scheduling_slots_for_date_v430($pdo,(int)$event['id'],$d,false);
        ...
        agent_tool_log($user,'scheduling.availability',$query,'success',['event_type_id'=>(int)$event['id'],'schedule_id'=>$scheduleId,'days'=>array_keys($days),'agent_id'=>$agentId],$conversationId);return $result;
    }
    ...
    if($intent==='create'){
        ...
        if(!$event){
            $choices=array_map(static fn(array $e):string=>'• '.(string)$e['title'].' · '.(int)$e['duration_minutes'].' minutes',$events);
            $result['answer']=$choices?"Which appointment type should I book?\n".implode("\n",$choices):'Create an appointment type in Scheduling before I can book it.';return $result;
        }
        $date=agent_scheduling_tools_date_v460($query,$timezone);if($date===null){$result['answer']='What date should I book the '.(string)$event['title'].'?';return $result;}
        $time=agent_scheduling_tools_time_request_v460($query);if(!$time){$result['answer']='What time should I book it? I’ll check that against your live availability.';return $result;}
        $slots=agent_scheduling_slots_for_date_v430($pdo,(int)$event['id'],$date,false);$picked=agent_scheduling_tools_pick_slot_v460($slots,$time);
        ...
        if(empty($picked['slot'])){
            ...
            $result['answer']=$times?'That time is not open. Available times are: '.implode(', ',$times).'.':'There are no open slots for that appointment type on '.$date.'.';return $result;
        }
        $guest=agent_scheduling_tools_guest_name_v460($query);if($guest===''){$result['answer']='Who is this appointment with?';return $result;}
        $email=agent_scheduling_tools_email_v460($query);$slot=$picked['slot'];
        ...
        $prepared=agent_scheduling_tools_prepare_v460($user,$conversationId,$agentId,'create',['schedule_id'=>$scheduleId,'event_type_id'=>(int)$event['id'],'start_at_utc'=>(string)$slot['start_at_utc'],'guest_name'=>$guest,'guest_email'=>$email],$summary,$query);
        ...
        agent_tool_log($user,'scheduling.book',$query,'approval_required',['schedule_id'=>$scheduleId,'event_type_id'=>(int)$event['id'],'start_at_utc'=>(string)$slot['start_at_utc'],'agent_id'=>$agentId],$conversationId);return $result;
    }

    $found=agent_scheduling_tools_find_booking_v460($pdo,$user,$scheduleId,$query);$booking=$found['booking'];
    if(!$booking){
        $choices=agent_scheduling_tools_booking_choices_v460((array)$found['matches'],$timezone);
        $result['answer']=$choices?"I found more than one possible appointment. Tell me the booking number or the guest name:\n{$choices}":'I could not find a matching active appointment on this Agent’s schedule.';return $result;
    }
    ...
    if($intent==='cancel'){
        ...
        $prepared=agent_scheduling_tools_prepare_v460($user,$conversationId,$agentId,'cancel',['schedule_id'=>$scheduleId,'booking_id'=>(int)$booking['id']],$summary,$query);
        ...
        $result['approval']=['required'=>true,'approved'=>false,'expires_at'=>(int)$prepared['expires_at'],'risk'=>$prepared['risk'],'version'=>'v4.60'];
        agent_tool_log($user,'scheduling.cancel',$query,'approval_required',['booking_id'=>(int)$booking['id'],'schedule_id'=>$scheduleId,'agent_id'=>$agentId],$conversationId);return $result;
    }

    if($intent==='reschedule'){
        $event=agent_scheduling_event_type_v430($pdo,(int)$booking['event_type_id']);
        if(!$event||(int)$event['owner_user_id']!==(int)$user['id']||(int)$event['schedule_id']!==$scheduleId){$result['answer']='That appointment type is no longer available on this Agent’s schedule.';return $result;}
        $originalDate=agent_scheduling_tools_slot_label_v460((string)$booking['start_at_utc'],$timezone,'Y-m-d');$date=agent_scheduling_tools_date_v460($query,$timezone,$originalDate);
        $time=agent_scheduling_tools_time_request_v460($query);if(!$time){$result['answer']='What new time should I move the appointment to?';return $result;}
        $slots=agent_scheduling_tools_slots_excluding_v460($pdo,$event,(string)$date,(int)$booking['id']);$picked=agent_scheduling_tools_pick_slot_v460($slots,$time);
        if(!empty($picked['ambiguous'])){$result['answer']='I found both AM and PM possibilities. Which one do you mean?';return $result;}
        if(empty($picked['slot'])){
            $times=array_map(fn(array $s):string=>agent_scheduling_tools_slot_label_v460((string)$s['start_at_utc'],$timezone,'g:i A'),array_slice($slots,0,8));
            $result['answer']=$times?'That new time is not open. Available times are: '.implode(', ',$times).'.':'There are no open times on '.(string)$date.'.';return $result;
        }
        $slot=$picked['slot'];$before=agent_scheduling_tools_slot_label_v460((string)$booking['start_at_utc'],$timezone);$after=agent_scheduling_tools_slot_label_v460((string)$slot['start_at_utc'],$timezone);
        $summary=(string)$booking['event_title'].' with '.(string)$booking['guest_name'].' · '.$before.' → '.$after;
        $prepared=agent_scheduling_tools_prepare_v460($user,$conversationId,$agentId,'reschedule',['schedule_id'=>$scheduleId,'booking_id'=>(int)$booking['id'],'start_at_utc'=>(string)$slot['start_at_utc']],$summary,$query);
        $result['answer']="I’m ready to reschedule:\n• {$summary}\n\nReply **Confirm reschedule** to approve it, or **Never mind** to stop.";
        $result['approval']=['required'=>true,'approved'=>false,'expires_at'=>(int)$prepared['expires_at'],'risk'=>$prepared['risk'],'version'=>'v4.60'];
        agent_tool_log($user,'scheduling.reschedule',$query,'approval_required',['booking_id'=>(int)$booking['id'],'schedule_id'=>$scheduleId,'start_at_utc'=>(string)$slot['start_at_utc'],'agent_id'=>$agentId],$conversationId);return $result;
    }
    return $empty;
}

/** Safe public Profile Agent skill: live reads only; mutations stay in Phase 3's bearer-token booking UI. */
function agent_scheduling_tools_profile_query_v460(PDO $pdo,string $query,array $profile,array $agent): ?string
{
    $ownerId=(int)($profile['user_id']??0);if($ownerId<1||!agent_scheduling_tools_ready_v460($pdo))return null;
    $schedule=agent_scheduling_public_schedule_v450($pdo,$ownerId);if(!$schedule)return null;
    $events=agent_scheduling_public_events_v450($pdo,(int)$schedule['id']);$intent=agent_scheduling_tools_intent_v460($query,$events);
    if(!in_array($intent,['availability','create','reschedule','cancel','list'],true))return null;
    $username=profile_username_normalize((string)($profile['username']??''));if($username==='')return null;
    $agentName=trim((string)($agent['display_name']??''))?:'the Profile Agent';$event=agent_scheduling_tools_event_v460($pdo,['id'=>$ownerId],$events,$query);
    if($intent==='availability'&&$event){
        $timezone=agent_scheduling_timezone_v430((string)$schedule['timezone']);$date=agent_scheduling_tools_date_v460($query,$timezone);
        $start=$date!==null?new DateTimeImmutable($date,new DateTimeZone($timezone)):new DateTimeImmutable('today',new DateTimeZone($timezone));$lines=[];$days=$date!==null?1:7;
        for($i=0;$i<$days&&count($lines)<3;$i++){
            $d=$start->modify('+'.$i.' days')->format('Y-m-d');$slots=agent_scheduling_slots_for_date_v430($pdo,(int)$event['id'],$d,true);if(!$slots)continue;
            $times=array_map(fn(array $s):string=>agent_scheduling_tools_slot_label_v460((string)$s['start_at_utc'],$timezone,'g:i A'),array_slice($slots,0,5));$lines[]=(new DateTimeImmutable($d))->format('D, M j').' — '.implode(', ',$times);
        }
    ...
    if($intent==='create')return 'I can help you schedule with '.trim((string)($profile['display_name']??$username)).'. Use **Book a time** on this profile to choose a live slot and confirm your contact details securely.';
    if(in_array($intent,['reschedule','cancel'],true))return 'Use the private appointment-management link from your booking confirmation to '.($intent==='cancel'?'cancel':'reschedule').' securely. I won’t expose or guess another visitor’s booking from public chat.';
