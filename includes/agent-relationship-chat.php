...
function vp3_agent_relationship_chat_subject(string $query): string
{
    ...
    $patterns=[
        ...
        '/\b(?:show|explain|review)\s+(?:the\s+)?(?:relationship|relationship score|opportunity|value|cost)\s+(?:for|of)\s+(.+?)[?.!]*$/i',
...
function vp3_agent_relationship_chat_tool(string $query,array $user,int $conversationId=0): array
{
    ...
    if(str_contains($q,'expensive')||str_contains($q,'costliest')||str_contains($q,'agent cost'))$mode='cost';
    ...
    if(!$top){
        $answer=$mode==='allow'?'I do not have a user-directed Agent Radar contact that currently clears the advisory allow threshold. Keep existing Gateway defaults in place until the relationship has more evidence.':'No scored Agent Radar contacts match that relationship view yet.';
    ...
    $heading=match($mode){'cost'=>'Highest-cost Agent Radar relationships:','value'=>'Highest-value Agent Radar relationships:','allow'=>'Best current candidates for review before allowing Agent Messaging: ',default=>'Top Agent Radar opportunities:'};
    $lines=[$heading];
    foreach($top as $row){
        ...
        if(trim((string)$row['inferred_intent'])!=='')$line.=' · '.str_replace('_',' ',(string)$row['inferred_intent']);
        $lines[]=$line.'.';
    }
