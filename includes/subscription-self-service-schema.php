...
function subscription_self_service_schema_ready(?PDO $pdo=null): bool
{
    $pdo??=db();
    return (bool)$pdo&&table_exists('subscription_plan_requests');
}

function subscription_self_service_ensure_schema(?PDO $pdo=null): void
{
    $pdo??=db();
    if(!$pdo)throw new RuntimeException('Database connection is unavailable.');
    $pdo->exec("CREATE TABLE IF NOT EXISTS subscription_plan_requests (
