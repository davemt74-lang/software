...
const STONEFELLOW_CHAT_ONBOARDING_V241='chat-onboarding-packages-20260906';
...
function chat_onboarding_v241_tool(string $query,array $user): array
{
    ...
    $intent=(bool)preg_match('/\b(onboarding|setup|set up|package|plan|subscription|trial|tokens?|quota|usage|upgrade|stem editor|video editor|profile agent|voice clone|team seats?|what.*missing|finish.*setup)\b/u',$q);if(!$intent)return $empty;
    ...
    $result=$empty;$result['handled']=true;$result['sources'][]=['source'=>'account:onboarding-state','title'=>'Package and account setup state'];$pkg=$state['package']??[];$balance=$pkg['ai']??[];$cap=$state['capabilities']??[];
    ...
    if(preg_match('/\b(package|plan|subscription|trial|upgrade)\b/u',$q)){
        $result['answer']='You are on '.((string)($pkg['package_name']??'an unassigned package')).' ('.((string)($pkg['status']??'unknown')).'). Your package controls commercial features and AI capacity; Manager/Producer roles are separate workspace relationships.';
        if(!empty($pkg['is_trial'])&&!empty($pkg['period_end']))$result['answer'].=' Your trial ends '.date('M j, Y',strtotime((string)$pkg['period_end'])).'.';
        $result['actions'][]=['type'=>'open_url','label'=>'View Plan & AI Usage','url'=>url('/subscription.php')];return $result;
    }
    ...
    if(!empty($state['locked']))$result['answer'].=' Features outside your package are shown as upgrade options and do not count against completion.';
