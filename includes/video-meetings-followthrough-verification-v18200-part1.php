...
function video_meeting_followthrough_verification_ensure_v18200(PDO $pdo,array $monitor,array $execution): array
{
    ...
    if(!$row){
        $pdo->prepare("INSERT INTO video_meeting_followthrough_closures (monitor_id,execution_id,plan_id,handoff_id,agenda_item_id,meeting_id,owner_user_id,criteria_snapshot,criteria_hash,criteria_source,status) VALUES (?,?,?,?,?,?,?,?,?,?,'waiting_for_verification')")
            ->execute([$monitorId,(int)$execution['id'],(int)$criteria['plan_id']?:null,(int)$criteria['handoff_id']?:null,(int)$execution['agenda_item_id'],(int)$execution['meeting_id'],$owner,$criteriaText,$criteriaHash,(string)$criteria['source']]);
        $row=video_meeting_followthrough_verification_row_v18200($pdo,$owner,(int)$pdo->lastInsertId())?:throw new RuntimeException('Follow-through closure could not be created.');
        video_meeting_followthrough_verification_event_v18200($pdo,$row,'closure_created','','waiting_for_verification','Outcome verification started from the meeting follow-through plan.',['criteria_source'=>(string)$criteria['source']]);
