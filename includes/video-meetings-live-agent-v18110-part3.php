...
function video_meeting_live_agent_start_v18110(PDO $pdo,array $meeting,array $user): array
{
    ...
    if(!empty($cap['media_worker_available'])&&function_exists('video_meeting_livekit_agent_dispatch_v1800')){
        ...
        $media=[
            'ok'=>!empty($dispatch['ok']),'dispatched'=>!empty($dispatch['dispatched']),
            'reason'=>video_meeting_live_agent_text_v18110($dispatch['reason']??'',100),
        ];
    }
    $state=[
        'active'=>true,
        'session_id'=>'mla-'.substr(hash('sha256',(string)$meeting['public_id'].'|'.bin2hex(random_bytes(16))),0,32),
...
{
    $out=[];$seen=[];
    foreach(is_array($context)?$context:[] as $row){
        if(!is_array($row))continue;
        $source=video_meeting_live_agent_text_v18110($row['source']??'',300);$title=video_meeting_live_agent_text_v18110($row['title']??'',300);
        if($source===''||isset($seen[$source]))continue;$seen[$source]=true;$out[]=['source'=>$source,'title'=>$title];
