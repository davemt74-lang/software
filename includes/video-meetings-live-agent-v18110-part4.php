...
function video_meeting_live_agent_ask_v18110(PDO $pdo,array $meeting,array $user,string $question,string $clientTurnId): array
{
    ...
    $privateCompute=!empty($meetingContext['meeting_intelligence']['private_compute'])||(string)($plan['effective_preference']??'')==='homeserver_only';
    $agentContext=[
    ...
    $sessionId=(string)$state['session_id'];$turn=[
        ...
        'question'=>$question,'answer'=>$answer,'created_at'=>gmdate('c'),'private_ephemeral'=>$privateCompute,
        'route'=>(string)($plan['route']??'unknown'),'sources'=>video_meeting_live_agent_sources_v18110($result['context']??[]),
    ];
    $persisted=$turn;
    if($privateCompute){$persisted['question']='';$persisted['answer']='';$persisted['sources']=[];}
    $turns=is_array($state['turns']??null)?$state['turns']:[];$turns[]=$persisted;$state['turns']=array_slice($turns,-40);
