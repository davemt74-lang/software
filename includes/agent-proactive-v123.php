...
function agent_proactive_v123_evidence_candidates(array $user,string $surface,array $context,array $activity): array
{
    ...
    if(function_exists('agent_proactive_operations_v036_candidates')){
        try{
            $pdo=db();
            if($pdo instanceof PDO){
                foreach(agent_proactive_operations_v036_candidates($pdo,$user) as $item)if(is_array($item))$candidates[]=$item;
            }
        }catch(Throwable $e){}
    }
    return $candidates;
