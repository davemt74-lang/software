...
function vp3_cognitive_presentation_work_update_v230(PDO $pdo,array $user): ?array
{
    $uid=(int)($user['id']??0);
    if($uid<1||!table_exists('notifications'))return null;
    try{
        $s=$pdo->prepare("SELECT id,type,title,body,target_url,source_id,created_at
          FROM notifications
          WHERE user_id=? AND type IN ('workflow_update','workflow_needs_attention')
            AND source_type='agent_workflow_run'
        ...
        return [
          ...
          'target_url'=>vp3_cognitive_presentation_internal_url_v510((string)($row['target_url']??'')),
          'run_id'=>max(0,(int)($row['source_id']??0)),
          'created_at'=>(string)$row['created_at'],
          'priority'=>(string)$row['type']==='workflow_needs_attention'?'critical':'status',
        ];
    }catch(Throwable $e){
        return null;
    }
...
function vp3_cognitive_presentation_state_v510(PDO $pdo,array $user,string $namespace,int $agentId=0): array
{
    ...
    $workUpdate=vp3_cognitive_presentation_work_update_v230($pdo,$user);
    ...
    $supervision=function_exists('vp3_cognitive_supervision_activity_projection_v2460')
        ...
        :['build'=>'','focus'=>null,'issues'=>[],'counts'=>[]];
    ...
    $brief['supervision_focus']=$supervision['focus']??null;
    $brief['supervision_counts']=$supervision['counts']??[];
    $brief['autonomy_focus']=$autonomy['focus']??null;
    $brief['autonomy_counts']=$autonomy['counts']??[];
    ...
    $brief['optimization_focus']=$optimization['focus']??null;
    ...
    $brief['commitment_focus']=$commitmentProtection['focus']??null;
    $brief['commitment_counts']=$commitmentProtection['counts']??[];
    $brief['commitments']=$commitmentProtection['commitments']??[];
    $brief['economics_focus']=$economics['focus']??null;
    ...
    return [
        ...
        'digest'=>$digest,'voice_candidate'=>$voice,'homeserver_presence'=>$homeServerPresence,'work_update'=>$workUpdate,'attention'=>$attentionStatus,
