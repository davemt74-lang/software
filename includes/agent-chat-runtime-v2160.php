...
$query=trim((string)($input['message']??''));$conversationId=(int)($input['conversation_id']??0);$inputMode=(string)($input['input_mode']??'text');$inputMode=$inputMode==='voice'?'voice':'text';if($query==='')throw new RuntimeException('Enter a message.');if(mb_strlen($query)>6000)throw new RuntimeException('That message is too long.');$ephemeralProtocol=!empty($input['ephemeral_protocol']);
...
if(function_exists('vp3_live_session_note_action_v2370')){
    try{vp3_live_session_note_action_v2370($user,'user','chat.message_sent','Sent an Agent Chat message',[
        'surface'=>'chat','conversation_id'=>$conversationId,'agent_id'=>$activeAgentId,
        'knowledge_scope'=>knowledge_retrieval_v162_normalize_scope($input['knowledge_scope']??null),
    ]);}catch(Throwable $e){error_log('VP3 live-session Chat user action failed: '.$e->getMessage());}
}
...
foreach($context as $item){$source=(string)($item['source']??'');if(function_exists('chat_context_is_internal_source')&&chat_context_is_internal_source($source))continue;$entry=['source'=>$source,'title'=>(string)($item['title']??'')];if(str_starts_with($source,'knowledge:')){$kid=(int)substr($source,10);$knowledge=get_knowledge_item($kid);if($knowledge&&!empty($knowledge['file_path'])){$scope=(string)($knowledge['knowledge_scope']??'system');$owned=(int)($knowledge['created_by_user_id']??0)===$userId;if(($scope==='personal'&&$owned)||$scope==='system')$entry['url']=url('/knowledge-file.php?id='.$kid);}}$publicSources[]=$entry;}
$mediaLimit=track_is_playlist_request($query)?7:(track_is_next_request($query)?1:4);$answerMedia=track_media_from_answer($answer,$user,$mediaLimit);$queryMedia=track_media_suggestions($query,$user,$mediaLimit);$media=vp3_agent_chat_runtime_authorized_media_v2160(array_slice(track_merge_media_suggestions($answerMedia,$queryMedia),0,$mediaLimit),$context);$playlistTitle=$media?track_playlist_title($query):'';$toolSources=!empty($toolResult['sources'])&&is_array($toolResult['sources'])?$toolResult['sources']:[];$meetingSources=array_merge(video_meeting_memory_chat_sources_v18120($meetingMemory),video_meeting_commitment_command_chat_sources_v18230($meetingMemory));$publicSources=array_merge($publicSources,$toolSources,$meetingSources,[chat_execution_v019_source($execution)]);$publicSources=array_values(array_filter($publicSources,static function($item):bool{if(!is_array($item))return false;$source=(string)($item['source']??'');return !function_exists('chat_context_is_internal_source')||!chat_context_is_internal_source($source);}));$cardNamespace=vp3_cognitive_agent_namespace_v500($pdo,$user,$activeAgentId);$cardRequests=function_exists('vp3_cognitive_cards_chat_requests_v520')?vp3_cognitive_cards_chat_requests_v520($pdo,$user,$cardNamespace,$query,8):[];$toolActions=!empty($toolResult['actions'])&&is_array($toolResult['actions'])?$toolResult['actions']:[];$allActions=array_values(array_merge($toolActions,$responseActions));
if(!$turnPrepared&&function_exists('vp3_cognitive_turn_prepare_v2430')){
    try{
        $turnSeed=is_array($context??null)?$context:[];
        if(!$turnSeed&&function_exists('chat_policy_context_v236'))$turnSeed=chat_policy_context_v236($query,$user,$principal,$conversationId);
        if($agentContext&&function_exists('agent_surface_v131_context_item'))array_unshift($turnSeed,agent_surface_v131_context_item($agentContext));
        $turnPrepared=vp3_cognitive_turn_prepare_v2430(
            $pdo,$user,$principal,$query,$turnSeed,
            ['direct_user_turn'=>true,'surface'=>'chat','conversation_id'=>$conversationId]
        );
    }catch(Throwable $e){
        error_log('VP3 Cognitive Turn v24.30 runtime preparation failed: '.$e->getMessage());
    }
}
if($turnPrepared&&function_exists('vp3_cognitive_turn_finalize_v2430')){
    $turn=vp3_cognitive_turn_finalize_v2430(
        $turnPrepared,$answer,$allActions,$execution,!empty($toolResult['handled']),
        ['conversation_id'=>$conversationId]
    );
}
$messageContext=['sources'=>$publicSources,'knowledge'=>$knowledgeContext,'media'=>$media,'stem_media'=>!empty($toolResult['stem_media'])&&is_array($toolResult['stem_media'])?$toolResult['stem_media']:[],'actions'=>$allActions,'cards'=>$cardRequests,'playlist_title'=>$playlistTitle,'agent_context'=>$persistedAgentContext,'agent'=>['id'=>(int)$principal['agent_id'],'name'=>(string)$principal['display_name'],'kind'=>(string)$principal['kind']],'execution'=>$execution,'cognitive_turn'=>$turn];$contextJson=json_encode($messageContext,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);$assistantMessageId=0;if(!$ephemeralProtocol){$stmt=$pdo->prepare('INSERT INTO chat_messages (conversation_id,user_id,role,message,context_json) VALUES (?,NULL,?,?,?)');$stmt->execute([$conversationId,'assistant',$answer,$contextJson]);$assistantMessageId=(int)$pdo->lastInsertId();if($brainAllowed)agent_brain_archive_and_parse($user,$conversationId,$assistantMessageId,'assistant',$answer,$inputMode);$pdo->prepare('UPDATE chat_conversations SET updated_at=NOW() WHERE id=?')->execute([$conversationId]);}
if(function_exists('vp3_live_session_note_action_v2370')){
    try{
