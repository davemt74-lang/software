...
function campaigns_rewards_platform_ensure_schema_v100(?PDO $pdo=null): void
{
    ...
    $exec("CREATE TABLE IF NOT EXISTS campaign_reward_set_items (
      reward_set_id BIGINT UNSIGNED NOT NULL,reward_product_id BIGINT UNSIGNED NOT NULL,variant_id BIGINT UNSIGNED NOT NULL DEFAULT 0,quantity INT UNSIGNED NOT NULL DEFAULT 1,priority INT NOT NULL DEFAULT 100,conditions_json LONGTEXT NULL,PRIMARY KEY(reward_set_id,reward_product_id,variant_id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    if(function_exists('column_exists')&&column_exists('campaign_reward_set_items','variant_id')){
        $pdo->exec("UPDATE campaign_reward_set_items SET variant_id=0 WHERE variant_id IS NULL");
        $pdo->exec("ALTER TABLE campaign_reward_set_items MODIFY variant_id BIGINT UNSIGNED NOT NULL DEFAULT 0");
    }
