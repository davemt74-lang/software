...
function video_meeting_outcome_learning_source_v18170(string $value): string
{
    $value=strtolower(trim($value));
    return in_array($value,['manual','suggested','prep_unresolved_commitments','prep_historical_decisions','prep_relevant_context'],true)?$value:'other';
}
...
function video_meeting_outcome_learning_guidance_v18170(array $row): ?array
{
    ...
    $verified=max(0,min(10000,(int)($row['verified_rate_bps']??0)));$friction=max(0,min(10000,(int)($row['friction_rate_bps']??0)));$kind=(string)($row['action_kind']??'');$source=video_meeting_outcome_learning_source_v18170((string)($row['source_kind']??'manual'));$priority=video_meeting_agenda_normalize_priority_v18140((string)($row['priority']??'normal'));
