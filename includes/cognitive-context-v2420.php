...
{
    return [
        'current_state','physical_context','entity_graph','live_session','continuity','supervision','autonomy','portfolio','forecast','optimization','resource_budget','commitment_protection','budget_governance','value_roi','decision_calibration','economics','replanning','conversation','current_priorities','active_objects',
...
function vp3_cognitive_context_section_weight_v2420(string $section,bool $historyIntent=false): float
{
    ...
    return match($section){
        'physical_context'=>97.0,'budget_governance'=>96.05,'value_roi'=>96.04,'decision_calibration'=>96.03,'live_session'=>96.0,'commitment_protection'=>95.99,'economics'=>95.985,'replanning'=>95.97,'resource_budget'=>95.95,'optimization'=>95.9,'forecast'=>95.8,'portfolio'=>95.7,'autonomy'=>95.5,'supervision'=>95.0,'continuity'=>94.0,'current_priorities'=>92.0,'conversation'=>90.0,
...
function vp3_cognitive_context_authority_v2420(string $section,string $source): string
{
    return match($section){
        ...
        'physical_context'=>'tracky_cloud_physical_context_v271_projection',
        ...
        'decision_calibration'=>'cognitive_decision_calibration_v2580_projection',
...
function vp3_cognitive_context_assemble_v2420(
    ...
    if(function_exists('tracky_agent_context_items_v271')){
        try{foreach(tracky_agent_context_items_v271($pdo,$user,$namespace,$query,$options) as $physicalItem)$items[]=$physicalItem;}catch(Throwable $e){}
    }
    ...
    if(function_exists('vp3_cognitive_supervision_context_item_v2460')){
        try{$supervisionItem=vp3_cognitive_supervision_context_item_v2460($pdo,$user,$namespace);if($supervisionItem)$items[]=$supervisionItem;}catch(Throwable $e){}
    }
    if(function_exists('vp3_cognitive_autonomy_context_item_v2470')){
        try{$autonomyItem=vp3_cognitive_autonomy_context_item_v2470($pdo,$user,$namespace);if($autonomyItem)$items[]=$autonomyItem;}catch(Throwable $e){}
    }
    ...
    if(function_exists('vp3_cognitive_replanning_context_item_v2530')){
        try{$replanItem=vp3_cognitive_replanning_context_item_v2530($pdo,$user,$namespace);if($replanItem)$items[]=$replanItem;}catch(Throwable $e){}
    }
    if(function_exists('vp3_cognitive_commitment_context_item_v2540')){
        try{$commitmentItem=vp3_cognitive_commitment_context_item_v2540($pdo,$user,$namespace);if($commitmentItem)$items[]=$commitmentItem;}catch(Throwable $e){}
    }
    ...
    if(function_exists('vp3_cognitive_value_context_item_v2570')){
        try{$valueItem=vp3_cognitive_value_context_item_v2570($pdo,$user,$namespace);if($valueItem)$items[]=$valueItem;}catch(Throwable $e){}
    }
    if(function_exists('vp3_cognitive_decision_context_item_v2580')){
        try{$decisionItem=vp3_cognitive_decision_context_item_v2580($pdo,$user,$namespace);if($decisionItem)$items[]=$decisionItem;}catch(Throwable $e){}
    }
    ...
    $packet=[
        ...
        'privacy'=>[
            ...
            'raw_physical_perception_copied'=>false,
