...
{
    $pdo??=db();return (bool)($pdo&&agent_workflow_schema_ready_v1400($pdo)&&table_exists('agent_workflow_run_dependencies'));
}
function agent_work_dependencies_ensure_schema_v174(?PDO $pdo=null): void
{
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS agent_workflow_run_dependencies (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,owner_user_id INT UNSIGNED NOT NULL,run_id BIGINT UNSIGNED NOT NULL,depends_on_run_id BIGINT UNSIGNED NOT NULL,created_by VARCHAR(32) NOT NULL DEFAULT 'user',created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE KEY uq_agent_workflow_run_dependency (run_id,depends_on_run_id),INDEX idx_agent_workflow_run_dependency_owner (owner_user_id,run_id,id),INDEX idx_agent_workflow_run_dependency_reverse (owner_user_id,depends_on_run_id,id),CONSTRAINT fk_agent_workflow_run_dependency_run FOREIGN KEY (run_id) REFERENCES agent_workflow_runs(id) ON DELETE CASCADE,CONSTRAINT fk_agent_workflow_run_dependency_on_run FOREIGN KEY (depends_on_run_id) REFERENCES agent_workflow_runs(id) ON DELETE CASCADE,CONSTRAINT fk_agent_workflow_run_dependency_owner FOREIGN KEY (owner_user_id) REFERENCES users(id) ON DELETE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
}
function agent_work_dependencies_require_v174(PDO $pdo,array $user): int
{
    $uid=(int)($user['id']??0);if($uid<1||!has_permission('account.access',$user))throw new RuntimeException('Agent Work Dependencies are not available for this account.');if(!agent_work_dependencies_schema_ready_v174($pdo))throw new RuntimeException('Agent Work Dependencies are not installed yet. An administrator needs to run the Phase 17.4 upgrade.');return $uid;
}
function agent_work_dependencies_rows_v174(PDO $pdo,int $uid,int $runId): array
{
    if($uid<1||$runId<1||!agent_work_dependencies_schema_ready_v174($pdo))return [];$stmt=$pdo->prepare("SELECT d.id,d.run_id,d.depends_on_run_id,d.created_by,d.created_at,p.title,p.status,p.execution_target,p.updated_at FROM agent_workflow_run_dependencies d INNER JOIN agent_workflow_runs p ON p.id=d.depends_on_run_id AND p.owner_user_id=d.owner_user_id WHERE d.owner_user_id=? AND d.run_id=? ORDER BY d.id");$stmt->execute([$uid,$runId]);return $stmt->fetchAll()?:[];
}
function agent_work_dependents_rows_v174(PDO $pdo,int $uid,int $runId): array
{
    if($uid<1||$runId<1||!agent_work_dependencies_schema_ready_v174($pdo))return [];$stmt=$pdo->prepare("SELECT d.id,d.run_id,d.depends_on_run_id,d.created_by,d.created_at,r.title,r.status,r.execution_target,r.updated_at FROM agent_workflow_run_dependencies d INNER JOIN agent_workflow_runs r ON r.id=d.run_id AND r.owner_user_id=d.owner_user_id WHERE d.owner_user_id=? AND d.depends_on_run_id=? ORDER BY d.id");$stmt->execute([$uid,$runId]);return $stmt->fetchAll()?:[];
}
function agent_work_dependency_public_edge_v174(array $row,bool $dependent=false): array
{
    $id=$dependent?(int)($row['run_id']??0):(int)($row['depends_on_run_id']??0);return ['run_id'=>$id,'title'=>(string)($row['title']??''),'status'=>(string)($row['status']??''),'execution_target'=>(string)($row['execution_target']??'cloud'),'created_at'=>(string)($row['created_at']??''),'updated_at'=>(string)($row['updated_at']??'')];
}
...
{
    if(!agent_work_dependencies_schema_ready_v174($pdo))return true;if($uid<1||$runId<1)return false;$stmt=$pdo->prepare("SELECT COUNT(*) FROM agent_workflow_run_dependencies d INNER JOIN agent_workflow_runs p ON p.id=d.depends_on_run_id AND p.owner_user_id=d.owner_user_id WHERE d.owner_user_id=? AND d.run_id=? AND p.status<>'completed'");$stmt->execute([$uid,$runId]);return (int)$stmt->fetchColumn()===0;
}
function agent_work_dependency_blocked_count_v174(PDO $pdo,int $uid): int
{
    if($uid<1||!agent_work_dependencies_schema_ready_v174($pdo))return 0;$stmt=$pdo->prepare("SELECT COUNT(DISTINCT d.run_id) FROM agent_workflow_run_dependencies d INNER JOIN agent_workflow_runs r ON r.id=d.run_id AND r.owner_user_id=d.owner_user_id INNER JOIN agent_workflow_runs p ON p.id=d.depends_on_run_id AND p.owner_user_id=d.owner_user_id WHERE d.owner_user_id=? AND r.status NOT IN ('completed','cancelled','paused','approval_pending','failed') AND r.approval_status<>'pending' AND p.status<>'completed'");$stmt->execute([$uid]);return max(0,(int)$stmt->fetchColumn());
}
function agent_work_dependency_cycle_v174(PDO $pdo,int $uid,int $runId,int $dependsOnRunId): bool
{
    if($runId===$dependsOnRunId)return true;$frontier=[$dependsOnRunId];$seen=[];$iterations=0;while($frontier&&$iterations++<200){$batch=[];foreach($frontier as $id){$id=(int)$id;if($id>0&&!isset($seen[$id])){$seen[$id]=true;$batch[]=$id;}}if(!$batch)return false;$placeholders=implode(',',array_fill(0,count($batch),'?'));$stmt=$pdo->prepare("SELECT run_id,depends_on_run_id FROM agent_workflow_run_dependencies WHERE owner_user_id=? AND run_id IN ({$placeholders})");$stmt->execute(array_merge([$uid],$batch));$next=[];foreach($stmt->fetchAll()?:[] as $edge){$candidate=(int)($edge['depends_on_run_id']??0);if($candidate===$runId)return true;if($candidate>0&&!isset($seen[$candidate]))$next[]=$candidate;}$frontier=$next;}if($frontier)throw new RuntimeException('The dependency graph is too deep to validate safely.');return false;
}
