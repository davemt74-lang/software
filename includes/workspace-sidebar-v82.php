...
if ($workspaceSidebarUser && table_exists('track_favorites')) {
    try {
        ...
        if ($workspaceStmt) {$workspaceStmt->execute([(int)$workspaceSidebarUser['id']]);$workspaceSavedCount=(int)$workspaceStmt->fetchColumn();}
    } catch (Throwable $e) {}
}
...
<aside class="chat-sidebar workspace-main-sidebar" id="chatSidebar">
  <div class="chat-sidebar-top"><a class="chat-brand" href="<?= e(url(has_permission('chat.access',$workspaceSidebarUser)?'/chat.php':'/index.php')) ?>"><?= e(system_agent_name()) ?></a><button class="chat-icon-button mobile-only" id="closeChatSidebar" type="button" aria-label="Close menu">×</button></div>
  <div class="chat-sidebar-sections">
    <section class="chat-sidebar-nav-section" aria-label="<?= e(system_agent_name()) ?> workspace"><div class="chat-history-label">Explore</div><nav class="chat-sidebar-nav">
      ...
      <?php if($workspaceSavedCount>0):?><a class="chat-sidebar-nav-link" href="<?= e(url('/chat.php?view=saved')) ?>"><span>♥</span><strong>Saved Songs</strong></a><?php endif;?>
    ...
    <section class="chat-sidebar-nav-section" aria-label="Account workspace"><div class="chat-history-label">Workspace</div><nav class="chat-sidebar-nav">
      <?php if(has_permission('account.access',$workspaceSidebarUser)): ?>
        ...
        <?php if(personal_capability_has_v242('voice_profile.access',$workspaceSidebarUser)):?><a class="chat-sidebar-nav-link <?= $workspaceSidebarActive==='voice_profile'?'active':'' ?>" href="<?= e(url('/voice-profile.php')) ?>"><span>◌</span><strong>Voice Profile</strong></a><?php endif;?>
        <a class="chat-sidebar-nav-link" href="<?= e(url('/notifications.php')) ?>"><span>●</span><strong>Notifications</strong></a>
      <?php endif; ?>
      <?php if(has_permission('tracks.manage',$workspaceSidebarUser)||has_permission('track_notes.manage',$workspaceSidebarUser)||has_permission('producer.access',$workspaceSidebarUser)):?><a class="chat-sidebar-nav-link" href="<?= e(url('/admin/stems.php')) ?>"><span>≋</span><strong>Stem Studio</strong></a><?php endif;?>
      ...
      <?php if(has_permission('admin.access',$workspaceSidebarUser)):?><a class="chat-sidebar-nav-link" href="<?= e(url('/admin/index.php')) ?>"><span>◇</span><strong>Admin Dashboard</strong></a><?php endif;?>
    </nav></section>
  </div>
  <div class="chat-sidebar-footer"><a href="<?= e(url('/logout.php')) ?>">Log Out</a></div>
</aside>
