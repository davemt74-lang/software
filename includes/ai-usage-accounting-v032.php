...
{
    global $config;
    return is_array($config['ai_cost_rates']??null)?$config['ai_cost_rates']:[];
}
...
function ai_usage_accounting_v032_rate(string $provider,string $model): ?array
{
    ...
    foreach($keys as $key){
        ...
        if(!is_numeric($input)||!is_numeric($output)||(float)$input<0||(float)$output<0)continue;
...
function ai_usage_accounting_v032_cloud_ledger(PDO $pdo,int $userId,string $traceId): ?array
{
    ...
    try{$stmt=$pdo->prepare('SELECT id,credit_tokens_used,package_tokens_used,total_tokens FROM ai_usage_ledger WHERE user_id=? AND trace_id=? ORDER BY id DESC LIMIT 1');$stmt->execute([$userId,$traceId]);return $stmt->fetch()?:null;}catch(Throwable $e){return null;}
}
...
function ai_usage_accounting_v032_record(PDO $pdo,array $user,int $agentId,int $conversationId,array $execution): void
{
    ...
    try{
        ...
        $source=(string)($execution['source']??'vp3_retrieval');$allowed=['homeserver_local','user_provider','vp3_cloud','vp3_retrieval','vp3_tool'];if(!in_array($source,$allowed,true))$source='vp3_retrieval';
        $usage=is_array($execution['usage']??null)?$execution['usage']:[];$input=max(0,(int)($usage['prompt_tokens']??$usage['input_tokens']??0));$output=max(0,(int)($usage['completion_tokens']??$usage['output_tokens']??0));$total=max(0,(int)($usage['total_tokens']??($input+$output)));
...
{
    if($micros===null)return '—';if($micros===0)return '$0.00';$usd=$micros/1000000;return $usd<0.01?'$'.number_format($usd,4):'$'.number_format($usd,2);
}

function ai_usage_accounting_v032_format_aggregate_cost(int $micros,int $unknown): string
{
    if($micros<=0&&$unknown>0)return '—';$label=ai_usage_accounting_v032_format_cost(max(0,$micros));return $unknown>0?$label.' + unknown':$label;
}

function ai_usage_accounting_v032_user_state(PDO $pdo,int $userId,int $limit=100): array
{
    $empty=['total_requests'=>0,'total_tokens'=>0,'cloud_tokens'=>0,'local_requests'=>0,'cloud_requests'=>0,'estimated_cost_micros'=>0,'unknown_cost_requests'=>0,'today'=>[],'month'=>[],'by_source'=>[],'by_model'=>[],'by_agent'=>[],'recent'=>[]];if($userId<1||!ai_usage_accounting_v032_schema_ready($pdo))return $empty;
    $summary=$pdo->prepare("SELECT COUNT(*) total_requests,COALESCE(SUM(total_tokens),0) total_tokens,COALESCE(SUM(cloud_tokens_charged),0) cloud_tokens,COALESCE(SUM(source='homeserver_local'),0) local_requests,COALESCE(SUM(source='vp3_cloud'),0) cloud_requests,COALESCE(SUM(estimated_cost_micros),0) estimated_cost_micros,COALESCE(SUM(estimated_cost_micros IS NULL),0) unknown_cost_requests FROM ai_execution_ledger WHERE user_id=?");$summary->execute([$userId]);$s=$summary->fetch()?:[];
    ...
    $bySource=$pdo->prepare('SELECT source,COUNT(*) requests,SUM(total_tokens) total_tokens,SUM(cloud_tokens_charged) cloud_tokens,COALESCE(SUM(estimated_cost_micros),0) estimated_cost_micros,COALESCE(SUM(estimated_cost_micros IS NULL),0) unknown_cost_requests FROM ai_execution_ledger WHERE user_id=? GROUP BY source ORDER BY requests DESC,source');$bySource->execute([$userId]);
    $byModel=$pdo->prepare("SELECT provider,model,source,COUNT(*) requests,SUM(input_tokens) input_tokens,SUM(output_tokens) output_tokens,SUM(total_tokens) total_tokens,SUM(cloud_tokens_charged) cloud_tokens,COALESCE(SUM(estimated_cost_micros),0) estimated_cost_micros,COALESCE(SUM(estimated_cost_micros IS NULL),0) unknown_cost_requests FROM ai_execution_ledger WHERE user_id=? GROUP BY provider,model,source ORDER BY total_tokens DESC,requests DESC LIMIT 50");$byModel->execute([$userId]);
    $byAgent=[];if(table_exists('user_agents')){$stmt=$pdo->prepare("SELECT l.agent_id,COALESCE(a.display_name,'System Agent') agent_name,COUNT(*) requests,SUM(l.total_tokens) total_tokens,SUM(l.cloud_tokens_charged) cloud_tokens FROM ai_execution_ledger l LEFT JOIN user_agents a ON a.id=l.agent_id WHERE l.user_id=? GROUP BY l.agent_id,a.display_name ORDER BY requests DESC,total_tokens DESC LIMIT 50");$stmt->execute([$userId]);$byAgent=$stmt->fetchAll()?:[];}
    ...
    return array_merge($empty,['total_requests'=>(int)($s['total_requests']??0),'total_tokens'=>(int)($s['total_tokens']??0),'cloud_tokens'=>(int)($s['cloud_tokens']??0),'local_requests'=>(int)($s['local_requests']??0),'cloud_requests'=>(int)($s['cloud_requests']??0),'estimated_cost_micros'=>(int)($s['estimated_cost_micros']??0),'unknown_cost_requests'=>(int)($s['unknown_cost_requests']??0),'today'=>$today,'month'=>$month,'by_source'=>$bySource->fetchAll()?:[],'by_model'=>$byModel->fetchAll()?:[],'by_agent'=>$byAgent,'recent'=>$recent->fetchAll()?:[]]);
}

function ai_usage_accounting_v032_admin_state(PDO $pdo,int $limit=200): array
{
    $empty=['requests'=>0,'users'=>0,'tokens'=>0,'cloud_tokens'=>0,'local_requests'=>0,'estimated_cost_micros'=>0,'unknown_cost_requests'=>0,'recent'=>[],'by_source'=>[]];if(!ai_usage_accounting_v032_schema_ready($pdo))return $empty;
    ...
    $recent=$pdo->query("SELECT l.*,u.display_name user_name,u.email user_email,a.display_name agent_name FROM ai_execution_ledger l INNER JOIN users u ON u.id=l.user_id LEFT JOIN user_agents a ON a.id=l.agent_id ORDER BY l.id DESC LIMIT ".max(1,min(500,$limit)))->fetchAll()?:[];
    $by=$pdo->query('SELECT source,COUNT(*) requests,SUM(total_tokens) total_tokens,SUM(cloud_tokens_charged) cloud_tokens,COALESCE(SUM(estimated_cost_micros),0) estimated_cost_micros,COALESCE(SUM(estimated_cost_micros IS NULL),0) unknown_cost_requests FROM ai_execution_ledger GROUP BY source ORDER BY requests DESC,source')->fetchAll()?:[];
...
{
    if(!ai_usage_accounting_v032_schema_ready($pdo))return [];$limit=max(1,min(50000,$limit));if($userId!==null){$stmt=$pdo->prepare('SELECT * FROM ai_execution_ledger WHERE user_id=? ORDER BY id DESC LIMIT '.$limit);$stmt->execute([$userId]);return $stmt->fetchAll()?:[];}return $pdo->query('SELECT * FROM ai_execution_ledger ORDER BY id DESC LIMIT '.$limit)->fetchAll()?:[];
}
