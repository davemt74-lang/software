...
{
    if(!agent_appointment_lifecycle_schema_ready_v700($pdo))return ['queued'=>0,'processed'=>0,'reconciled'=>0,'followups_created'=>0,'followups_executed'=>0];
...
function agent_appointment_lifecycle_housekeeping_maybe_v700(): void
{
    ...
    if(strtoupper((string)($_SERVER['REQUEST_METHOD']??'GET'))==='POST'){
        register_shutdown_function(static function():void{
            try{
                $db=db();
                if($db&&!$db->inTransaction()&&agent_appointment_lifecycle_schema_ready_v700($db)){
                    agent_appointment_lifecycle_reconcile_statuses_v700($db,120);
                    $due=$db->query("SELECT * FROM agent_scheduling_automation_deliveries WHERE status='pending' AND due_at<=UTC_TIMESTAMP() AND attempts<3 ORDER BY due_at,id LIMIT 30")->fetchAll()?:[];
