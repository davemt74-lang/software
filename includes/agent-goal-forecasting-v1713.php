...
function agent_goal_forecast_history_v1713(PDO $pdo,int $uid,string $goalText): array
{
    ...
    try{
        $stmt=$pdo->prepare("SELECT id,goal,created_at,updated_at,status,objective_verification_status FROM agent_workflow_runs WHERE owner_user_id=? AND source_kind='objective_plan' AND objective_verification_status='achieved' ORDER BY id DESC LIMIT ".VP3_AGENT_GOAL_FORECAST_HISTORY_LIMIT_V1713);
...
{
    if($forecastDate==='')return ['status'=>'insufficient_evidence','slip_days'=>null,'reason'=>'There is not enough roadmap evidence to produce a completion date yet.'];
...
function agent_goal_forecast_state_v1713(PDO $pdo,array $user,int $goalId,?string $comparisonDate=null): array
{
    ...
    $achieved=(string)($goal['derived_status']??'')==='achieved';$remaining=max(0,(int)($plan['plan']['remaining_count']??0));$estimable=$achieved||$remaining>0;if($achieved)$days=0.0;
    $capacity=agent_goal_capacity_v1713($pdo,$user);$capacityBuffer=0.0;if($estimable&&!$achieved){if((string)$capacity['label']==='moderate')$capacityBuffer=$days*0.08;elseif((string)$capacity['label']==='high')$capacityBuffer=$days*0.18;elseif((string)$capacity['label']==='overcommitted')$capacityBuffer=$days*0.30;$days+=$capacityBuffer;}
    $forecastDays=$estimable?max(0,(int)ceil($days)):0;$today=strtotime(gmdate('Y-m-d').' 00:00:00 UTC')?:time();$forecastDate=$estimable?gmdate('Y-m-d',$today+($forecastDays*86400)):'';$target=trim((string)($comparisonDate??''));if($target==='')$target=trim((string)($goal['target_date']??''));
    $risk=agent_goal_forecast_risk_v1713($forecastDate,$target);$confidence=max(20,min(95,(int)$history['confidence_percent']));if(!$estimable)$confidence=min($confidence,25);elseif($achieved)$confidence=95;if($remaining>8)$confidence=max(20,$confidence-8);$confidenceLabel=$confidence>=75?'high':($confidence>=50?'medium':'low');
    $scenarios=$estimable?agent_goal_forecast_scenarios_v1713($forecastDays,$target):[];$state=['build'=>VP3_AGENT_GOAL_FORECASTING_V1713,'goal'=>$goal,'plan'=>$plan,'execution'=>$execution,'history'=>$history,'capacity'=>$capacity,'work'=>$work,'forecast'=>['estimable'=>$estimable,'forecast_days'=>$forecastDays,'forecast_date'=>$forecastDate,'baseline_days_per_objective'=>$baseline,'capacity_buffer_days'=>round($capacityBuffer,2),'confidence_percent'=>$confidence,'confidence_label'=>$confidenceLabel,'comparison_date'=>$target],'risk'=>$risk,'scenarios'=>$scenarios];$state['recovery']=agent_goal_forecast_recovery_v1713($state);return $state;
}
...
function agent_goal_forecast_answer_v1713(array $state,string $mode='forecast'): string
{
    ...
    if(empty($forecast['estimable'])){$answer='Goal #'.$id.' forecast: insufficient roadmap evidence for a completion date. '.(string)($risk['reason']??'');$recovery=(array)($state['recovery']??[]);if($recovery)$answer.=' Next: '.$recovery[0];return $answer.' Forecasting will not invent zero remaining work or mutate the roadmap.';}
    if($mode==='scenarios'){
