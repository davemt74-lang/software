 ...
 *
 * Phase 1 of the native scheduling subsystem. This is deliberately modeled as
 * an Agent capability rather than a standalone calendar: an account can own
 * multiple public schedules, optionally bind each schedule to a VP3 Agent,
 * publish event types, define recurring/date-specific availability, and create
 * conflict-safe bookings. External calendar sync and conversational Agent tools
 * build on this canonical store in later phases.
 */
...
function agent_scheduling_conflict_v430(
    PDO $pdo,
    int $scheduleId,
    string $startUtc,
    string $endUtc,
    ...
    $sql = "SELECT id,start_at_utc,end_at_utc,status
            ...
            WHERE schedule_id=?
              ...
              AND start_at_utc < ?
              AND end_at_utc > ?";
    $args = [$scheduleId, $endUtc, $startUtc];
...
function agent_scheduling_create_booking_v430(PDO $pdo, array $input): array
{
    ...
    if (!$event || empty($event['is_active']) || empty($event['schedule_active'])) {
        throw new RuntimeException('This appointment type is not available.');
    }
    ...
    if ($guestEmail !== '' && !filter_var($guestEmail, FILTER_VALIDATE_EMAIL)) {
        throw new RuntimeException('Enter a valid attendee email.');
    }
    ...
    // MySQL named locks serialize booking creation per schedule. An ordinary
    // overlap SELECT alone can race when two requests target an empty slot.
    $lockName = 'vp3_schedule_' . $scheduleId;
    $lockStmt = $pdo->prepare('SELECT GET_LOCK(?,5)');
    $lockStmt->execute([$lockName]);
    if ((int)$lockStmt->fetchColumn() !== 1) {
        throw new RuntimeException('That schedule is busy. Please choose the time again.');
    }

    try {
        $conflict = agent_scheduling_conflict_v430($pdo, $scheduleId, $startUtc, $endUtc);
        if ($conflict) throw new RuntimeException('That time was just booked. Please choose another time.');

        $publicToken = bin2hex(random_bytes(32));
        $cancelToken = bin2hex(random_bytes(32));
        $stmt = $pdo->prepare(
            'INSERT INTO agent_scheduling_bookings
             (owner_user_id,schedule_id,event_type_id,agent_id,created_by_user_id,created_by_agent_id,event_title,duration_minutes,start_at_utc,end_at_utc,organizer_timezone,guest_timezone,guest_name,guest_email,guest_phone,status,location_type,location_value,guest_notes,internal_notes,source,public_token,cancel_token)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
        );
        $stmt->execute([
            $ownerUserId,
            ...
            $createdByUserId,
            $createdByAgentId,
            mb_strimwidth((string)$event['title'], 0, 190, ''),
            $duration,
            $startUtc,
            $endUtc,
            $organizerTimezone,
            $guestTimezone,
            mb_strimwidth($guestName, 0, 190, ''),
            mb_strimwidth($guestEmail, 0, 190, ''),
            mb_strimwidth(trim((string)($input['guest_phone'] ?? '')), 0, 80, ''),
            'confirmed',
            mb_strimwidth((string)($event['location_type'] ?? 'virtual'), 0, 30, ''),
            mb_strimwidth((string)($event['location_value'] ?? ''), 0, 500, ''),
            trim((string)($input['guest_notes'] ?? '')) ?: null,
            trim((string)($input['internal_notes'] ?? '')) ?: null,
            $source,
            $publicToken,
            $cancelToken,
        ]);
...
function agent_scheduling_cancel_booking_v430(
    PDO $pdo,
    int $bookingId,
    ...
    if ($ownerUserId !== null && $ownerUserId > 0) {
        $stmt = $pdo->prepare(
            "UPDATE agent_scheduling_bookings
             ...
             WHERE id=? AND owner_user_id=? AND status IN ('pending','confirmed')"
        );
