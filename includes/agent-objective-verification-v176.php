...
function agent_objective_verification_rewire_failed_v176(PDO $pdo,array $user,int $objectiveRunId,array $onlyRunIds=[]): array
{
    ...
    $only=array_fill_keys(array_values(array_unique(array_filter(array_map('intval',$onlyRunIds),static fn(int $id): bool=>$id>0))),true);
    ...
    if($only)$failed=array_values(array_filter($failed,static fn(array $row): bool=>isset($only[(int)($row['id']??0)])));
    ...
    if(!$replacements)return ['replaced'=>0,'replacement_run_ids'=>[]];
