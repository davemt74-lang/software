...
{
    $pdo??=db();
    if(!$pdo||$userId<1||!subscription_schema_ready($pdo))return null;
...
function subscription_self_service_select_plan(array $user,int $packageId,string $interval='monthly'): array
{
    ...
    $pdo=db();if(!$pdo||!subscription_schema_ready($pdo))throw new RuntimeException('Plan management is unavailable until the database upgrade is complete.');
    $interval=subscription_self_service_interval($interval);
...
function subscription_self_service_cancel_request(array $user,int $requestId): void
{
    ...
    $pdo=db();if(!$pdo||!subscription_schema_ready($pdo))throw new RuntimeException('Plan management is unavailable.');
    $ownsTransaction=!$pdo->inTransaction();if($ownsTransaction)$pdo->beginTransaction();
    try{
...
{
    $pdo??=db();if(!$pdo||$userId<1||!subscription_schema_ready($pdo))return 0;
    ...
    foreach($ids as $requestId){
        ...
        try{
            ...
            $current=subscription_current_for_user_id($userId,$pdo,true);
            if((int)($request['subscription_id']??0)>0&&(!$current||(int)$current['id']!==(int)$request['subscription_id'])){
