...
{
    $ownerId=(int)($user['id']??0);if($ownerId<1||!agent_team_scheduling_schema_ready_v600($pdo))return null;
...
{
    $empty=agent_team_scheduling_tools_empty_v610();$pdo=db();if(!$pdo||!agent_team_scheduling_schema_ready_v600($pdo)||(int)($user['id']??0)<1)return $empty;
