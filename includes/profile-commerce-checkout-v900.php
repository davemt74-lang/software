...
function profile_commerce_checkout_nonce_v900(int $ownerUserId,int $productId): string
{
    ...
    $_SESSION['profile_commerce_checkout_intents'][$nonce]=['owner_user_id'=>$ownerUserId,'product_id'=>$productId,'order_id'=>0,'connection_id'=>0,'created_at'=>$now,'expires_at'=>$now+7200];
    return $nonce;
}
...
function profile_commerce_create_checkout_idempotent_v900(PDO $pdo,array $profile,array $projectedProduct,int $connectionId,string $payerEmail,string $nonce,string $termsDigest,bool $termsAccepted): array
{
    ...
    if($storedOrder>0){
        ...
        $order=agent_commerce_create_order_v800($pdo,$product,['connection_id'=>(int)$selected['id'],'payer_email'=>$payerEmail,'metadata'=>['source'=>'profile_commerce_v900','profile_username'=>(string)$profile['username'],'profile_product_slug'=>(string)$projectedProduct['slug'],'checkout_nonce_sha256'=>hash('sha256',$nonce),'terms_accepted_at'=>gmdate('c'),'terms_snapshot_sha256'=>$currentTermsDigest]]);
