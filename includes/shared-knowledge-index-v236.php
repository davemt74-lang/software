...
function shared_knowledge_index_tags_v236(string $text,int $limit=36): string
{
    ...
    foreach($tokens as $token){
        $token=trim($token);if(mb_strlen($token)<3||isset($stop[$token]))continue;
...
{
    $stmt=$pdo->prepare('SELECT id,title,description,content_text,visibility,is_published,created_by_user_id,updated_at FROM knowledge_items WHERE id=? LIMIT 1');
    $stmt->execute([$knowledgeId]);$item=$stmt->fetch();if(!$item)return null;
    $chunks=[];
    if(table_exists('knowledge_chunks')){
        $c=$pdo->prepare('SELECT chunk_text FROM knowledge_chunks WHERE knowledge_id=? ORDER BY chunk_index ASC,id ASC');$c->execute([$knowledgeId]);$chunks=$c->fetchAll(PDO::FETCH_COLUMN)?:[];
    }
...
function shared_knowledge_index_hash_v236(array $item): string
{
    return hash('sha256',json_encode([
      ...
      'chunks'=>array_values((array)($item['chunk_texts']??[])),
      'visibility'=>(string)($item['visibility']??''),
      'is_published'=>(int)($item['is_published']??0),
...
{
    // Keep only ownership/pointer/timestamps for audit. Search terms, title,
    // embedding reference and source-derived hash are erased on revocation.
...
function shared_knowledge_index_sync_item_v236(PDO $pdo,int $knowledgeId): void
{
    ...
    $owner=(int)($item['created_by_user_id']??0);
    if($owner<1){$pdo->prepare('DELETE FROM shared_knowledge_index WHERE knowledge_id=?')->execute([$knowledgeId]);return;}
    ...
    $hash=shared_knowledge_index_hash_v236($item);
    $tags=shared_knowledge_index_tags_v236($searchText);
    $scope=(string)($policy['audience_scope']??'inherit');
...
function shared_knowledge_index_sync_owner_v236(PDO $pdo,int $ownerUserId): void
{
    ...
    if(!shared_knowledge_index_schema_ready_v236($pdo))shared_knowledge_index_ensure_schema_v236($pdo);
    $stmt=$pdo->prepare('SELECT id FROM knowledge_items WHERE created_by_user_id=? ORDER BY id ASC');$stmt->execute([$ownerUserId]);
    $ids=array_map('intval',$stmt->fetchAll(PDO::FETCH_COLUMN)?:[]);
    foreach($ids as $id)shared_knowledge_index_sync_item_v236($pdo,$id);
    if($ids){
        $marks=implode(',',array_fill(0,count($ids),'?'));$args=array_merge([$ownerUserId],$ids);
        $stale=$pdo->prepare("SELECT knowledge_id FROM shared_knowledge_index WHERE owner_user_id=? AND knowledge_id NOT IN ({$marks})");$stale->execute($args);
        foreach($stale->fetchAll(PDO::FETCH_COLUMN)?:[] as $id)shared_knowledge_index_revoke_v236($pdo,(int)$id);
    }else{
        $stale=$pdo->prepare('SELECT knowledge_id FROM shared_knowledge_index WHERE owner_user_id=?');$stale->execute([$ownerUserId]);
        foreach($stale->fetchAll(PDO::FETCH_COLUMN)?:[] as $id)shared_knowledge_index_revoke_v236($pdo,(int)$id);
    }
}
...
{
    if(!shared_knowledge_index_schema_ready_v236($pdo)||!table_exists('user_data_policies')||!table_exists('knowledge_items'))return;
    $limit=max(1,min(200,$limit));
    // Exact item policy overrides the owner's wildcard policy. We refresh only
    // published items that are currently shared and are missing/stale/revoked.
    $sql="SELECT k.id
          FROM knowledge_items k
          LEFT JOIN user_data_policies pe
            ON pe.owner_user_id=k.created_by_user_id
           AND pe.resource_type='knowledge'
           AND pe.resource_id=CAST(k.id AS CHAR)
          LEFT JOIN user_data_policies pw
            ON pw.owner_user_id=k.created_by_user_id
           AND pw.resource_type='knowledge'
           AND pw.resource_id='*'
...
{
    if(!shared_knowledge_index_schema_ready_v236($pdo))return [];
    ...
    // A stale pointer can never surface an unpublished source: current source
    // publication state is checked in SQL before the candidate is returned.
    $rows=$pdo->query('SELECT i.knowledge_id,i.owner_user_id,i.source_version_hash,i.title,i.topic_tags,i.share_scope,i.last_indexed_at FROM shared_knowledge_index i INNER JOIN knowledge_items k ON k.id=i.knowledge_id AND k.is_published=1 WHERE i.is_indexed=1 AND i.revoked_at IS NULL ORDER BY i.updated_at DESC,i.knowledge_id DESC LIMIT 500')->fetchAll()?:[];
    if(!$terms)return array_slice($rows,0,$limit);
    $scored=[];
    foreach($rows as $row){$hay=mb_strtolower((string)$row['title'].' '.(string)$row['topic_tags']);$score=0;foreach($terms as $term){if(str_contains($hay,$term))$score+=str_contains(mb_strtolower((string)$row['title']),$term)?3:1;}if($score>0)$scored[]=['score'=>$score,'row'=>$row];}
