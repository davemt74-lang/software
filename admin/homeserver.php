...
<div class="admin-grid admin-grid-2">
  ...
  <section class="admin-card">
    <div class="admin-card-head"><div><h3>Upload HomeServer</h3><p>Maximum 512 MB per executable.</p></div></div>
    <form method="post" enctype="multipart/form-data" class="admin-form">
      ...
      <div class="form-row">
        ...
        <label>Channel<select name="channel"><option value="stable">Stable</option><option value="beta">Beta</option><option value="dev">Development</option></select></label>
      </div>
      ...
      <div class="admin-check-grid">
        ...
        <label><input type="checkbox" name="is_latest" value="1" checked> Make current for channel</label>
      </div>
...
<section class="admin-card" style="margin-top:18px">
  ...
  <div class="admin-table-wrap"><table class="admin-table">
    ...
    <tbody>
    <?php foreach ($releases as $release): $id=(int)$release['id']; ?>
      <tr id="release-<?= $id ?>">
        ...
        <td><?php if((string)$release['portable_path']!==''): ?><a href="<?= e(url('/homeserver-download.php?id='.$id.'&type=portable')) ?>">Download</a><br><small><?= number_format((int)$release['portable_size']) ?> bytes<br><?= e(substr((string)$release['portable_sha256'],0,16)) ?>…</small><?php else: ?>—<?php endif; ?></td>
        <td><?php if((string)$release['installer_path']!==''): ?><a href="<?= e(url('/homeserver-download.php?id='.$id.'&type=installer')) ?>">Download</a><br><small><?= number_format((int)$release['installer_size']) ?> bytes<br><?= e(substr((string)$release['installer_sha256'],0,16)) ?>…</small><?php else: ?>—<?php endif; ?></td>
        <td><?= (int)$release['is_published']===1?'Published':'Draft' ?><?= (int)$release['is_latest']===1?' · Current':'' ?></td>
        <td>
          <div class="form-actions">
            <?php if((int)$release['is_published']!==1): ?><form method="post"><?= csrf_field() ?><input type="hidden" name="action" value="publish"><input type="hidden" name="release_id" value="<?= $id ?>"><button class="button button-small" type="submit">Publish</button></form><?php endif; ?>
            ...
            <?php if((int)$release['is_published']===1): ?><form method="post"><?= csrf_field() ?><input type="hidden" name="action" value="unpublish"><input type="hidden" name="release_id" value="<?= $id ?>"><button class="button button-small" type="submit">Unpublish</button></form><?php endif; ?>
