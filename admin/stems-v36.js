(() => {
  ...
  function createEq5Graph(plugin) {
    ...
    const filters = frequencies.map((frequency, index) => {
      ...
      node.Q.value = index === 0 || index === frequencies.length - 1
        ? 0.7
        : 1.0;
      node.gain.value = Number(plugin.params[keys[index]] || 0);
    ...
    return `
      <label class="daw-plugin-param">
        ...
        <input
          ...
          step="${step}"
          value="${value}"
          data-plugin-param="${key}"
        >
        <output data-plugin-param-output="${key}">${Number(value).toFixed(step < 1 ? 2 : 1)}${suffix}</output>
      </label>
    `;
  }
  ...
  function openPluginEditor(stemId, index) {
    ...
    if (pluginEditorControls) {
      if (plugin.type === 'eq5') {
        pluginEditorControls.innerHTML = `
          <div class="daw-eq5-editor">
            ${editorRange({label:'80 Hz',key:'b1',min:-18,max:18,step:0.5,value:plugin.params.b1,suffix:' dB'})}
            ${editorRange({label:'250 Hz',key:'b2',min:-18,max:18,step:0.5,value:plugin.params.b2,suffix:' dB'})}
            ${editorRange({label:'1 kHz',key:'b3',min:-18,max:18,step:0.5,value:plugin.params.b3,suffix:' dB'})}
            ${editorRange({label:'4 kHz',key:'b4',min:-18,max:18,step:0.5,value:plugin.params.b4,suffix:' dB'})}
            ${editorRange({label:'12 kHz',key:'b5',min:-18,max:18,step:0.5,value:plugin.params.b5,suffix:' dB'})}
          </div>
        `;
      } else {
        pluginEditorControls.innerHTML = `
          <div class="daw-delay-editor">
            ${editorRange({label:'Time',key:'time',min:0.02,max:1.5,step:0.01,value:plugin.params.time,suffix:' s'})}
            ${editorRange({label:'Feedback',key:'feedback',min:0,max:0.92,step:0.01,value:plugin.params.feedback})}
            ${editorRange({label:'Mix',key:'mix',min:0,max:1,step:0.01,value:plugin.params.mix})}
          </div>
        `;
      }

      pluginEditorControls
        .querySelectorAll('[data-plugin-param]')
        .forEach(input => {
          input.addEventListener('input', () => {
            const currentStem = stemById(pluginTargetStemId);
            const currentPlugin = currentStem?.plugins?.[pluginEditIndex];

            if (!currentPlugin) return;

            const key = input.dataset.pluginParam;
            currentPlugin.params[key] = Number(input.value);

            const output = pluginEditorControls.querySelector(
              `[data-plugin-param-output="${key}"]`
            );

            if (output) {
              const suffix = currentPlugin.type === 'eq5'
                ? ' dB'
                : key === 'time'
                  ? ' s'
                  : '';
              ...
              output.textContent =
                `${Number(input.value).toFixed(2)}${suffix}`;
            }

            rebuildTrackPluginGraph(currentStem);
            renderTrackPluginList(currentStem);
            scheduleLocalSave();
          });
        });
    }

    document.getElementById('pluginDirectoryGrid')?.setAttribute('hidden', '');
    if (pluginEditor) {
