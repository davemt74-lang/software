(() => {
  ...
  async function run(file) {
    ...
    setProgress(94, 'Preparing stem library…');
    const preparedCommit = await request(base('stage_start'));
    ...
    while (committed < totalCommitStems) {
      const step = await request(base('stage_item'));
    ...
    const done = await request(base('stage_finish'));
