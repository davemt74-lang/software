(() => {
  ...
  async function run() {
    ...
    setProgress(96, 'Saving stem library…');
    const complete = await request(base('commit'));

    if (files.length) {
      setProgress(100, 'MP3 stems imported.');
      setStatus(`${Number(complete.stem_count || files.length)} MP3 stems are ready.`);
    } else {
      setProgress(100, 'REAPER project metadata imported.');
      setStatus('REAPER project attached. Add MP3 stems whenever you are ready.');
    }
