(() => {
  ...
  async function run(file) {
    ...
    setProgress(94, 'Preparing stem library commit…');
    const preparedCommit = await request(base('commit_prepare'));
    const totalCommitStems = Number(
      preparedCommit.total_stems ?? chosen.audio.length
    );

    let committed = 0;

    while (committed < totalCommitStems) {
      const step = await request(base('commit_step'));
      committed = Number(step.completed || 0);

      setProgress(
        94 + (committed / totalCommitStems) * 5,
        `Saving stem ${committed} of ${totalCommitStems}` +
          (step.stem_name ? ` · ${step.stem_name}` : '')
      );
    }

    setProgress(99.4, 'Activating new stem library…');
    const done = await request(base('commit_finish'));
