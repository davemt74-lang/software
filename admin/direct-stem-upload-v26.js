(() => {
  ...
  async function run() {
    ...
    setProgress(94, 'Preparing production-file commit…');
    const preparedCommit = await request(base('commit_prepare'));
    const totalCommitStems = Number(preparedCommit.total_stems || 0);
    ...
    const complete = await request(base('commit_finish'));
