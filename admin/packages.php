    ...
    if(!verify_csrf())$error='Session expired. Please try again.';
        ...
        try{
            ...
            if($action==='save'){
                ...
                $values=[$slug,$name,trim((string)($_POST['description']??'')),max(0,(int)($_POST['monthly_price_cents']??0)),max(0,(int)($_POST['annual_price_cents']??0)),max(0,(int)($_POST['ai_tokens_monthly']??0)),max(0,min(3650,(int)($_POST['trial_days']??0))),max(0,(int)($_POST['trial_tokens']??0)),!empty($_POST['is_trial'])?1:0,!empty($_POST['is_default'])?1:0,!empty($_POST['is_public'])?1:0,!empty($_POST['is_active'])?1:0,(int)($_POST['sort_order']??100)];
...
$showModal=isset($_GET['new'])||$editId>0||$error!=='';
...
<section class="admin-card">
  <div class="admin-card-head"><div><h3>Package catalog</h3><p><?= count($packages) ?> configured package<?= count($packages)===1?'':'s' ?></p></div></div>
  <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Package</th><th>Pricing</th><th>AI allowance</th><th>Status</th><th></th></tr></thead><tbody>
    <?php foreach($packages as $row): ?><tr>
      ...
      <td><?php if((int)$row['monthly_price_cents']>0): ?>$<?= number_format((int)$row['monthly_price_cents']/100,2) ?>/mo<?php else: ?>Free<?php endif; ?><br><small><?php if((int)$row['annual_price_cents']>0): ?>$<?= number_format((int)$row['annual_price_cents']/100,2) ?>/yr<?php else: ?>No annual price<?php endif; ?></small></td>
    ...
    <?php if(!$packages): ?><tr><td colspan="5">No packages configured.</td></tr><?php endif; ?>
  </tbody></table></div>
</section>
...
<div class="admin-modal" id="packageModal" data-admin-modal aria-hidden="<?= $showModal?'false':'true' ?>" <?= $showModal?'data-admin-modal-auto-open="1"':'hidden' ?>>
  <div class="admin-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="packageModalTitle">
    ...
    <div class="admin-modal-foot">
      <?php if($package): ?><form method="post" style="margin-right:auto"><?= csrf_field() ?><input type="hidden" name="action" value="duplicate"><input type="hidden" name="package_id" value="<?= (int)$editId ?>"><button class="button" type="submit">Duplicate</button></form><?php endif; ?>
      <a class="button" href="<?= e(url('/admin/packages.php')) ?>" data-admin-modal-close>Cancel</a><button class="button primary" type="submit" form="packageForm">Save package</button>
    </div>
  </div>
</div>
