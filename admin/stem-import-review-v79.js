(() => {
  ...
  function nativePluginDefaults(type, role = 'Other') {
    const sourceRole = text(role).toLowerCase();

    if (type === 'eq5') {
      const params = {f1:80,f2:250,f3:1000,f4:4000,f5:12000,b1:0,b2:0,b3:0,b4:0,b5:0};
      if (sourceRole === 'vocal') Object.assign(params,{b1:-2,b2:-1,b3:1.2,b4:1.5,b5:0.8});
      else if (sourceRole === 'drums' || sourceRole === 'percussion') Object.assign(params,{b1:1.5,b2:-1,b3:0,b4:1,b5:1});
      else if (sourceRole === 'bass') Object.assign(params,{b1:2,b2:0.8,b3:-1.2,b4:-1,b5:-2});
      else if (sourceRole === 'guitar') Object.assign(params,{b1:-2,b2:-1,b3:1,b4:1.2,b5:0.5});
      return params;
    }

    if (type === 'compressor') {
      if (sourceRole === 'vocal') return {threshold:-18,ratio:3.5,knee:10,attack:0.008,release:0.18,makeup:1.5};
      if (sourceRole === 'drums' || sourceRole === 'percussion') return {threshold:-16,ratio:4,knee:8,attack:0.005,release:0.12,makeup:1};
      if (sourceRole === 'bass') return {threshold:-20,ratio:4, knee:10,attack:0.015,release:0.25,makeup:1.5};
      return {threshold:-18,ratio:3,knee:12,attack:0.012,release:0.24,makeup:0};
    }

    if (type === 'delay') return {time:0.28,feedback:0.32,mix:0.18};
    if (type === 'reverb') return {decay:1.8,size:1,damping:9000,mix:0.18};
    if (type === 'limiter') return {threshold:-6,ceiling:-1,release:0.10,lookahead:0.003};
    return {};
  }

  function nativePluginType(name) {
    const source = text(name).toLowerCase();
    if (/rea[- ]?comp|compressor|compression/.test(source)) return 'compressor';
    if (/reaeq|equalizer|equaliser|5[- ]?band|five[- ]?band|\beq\b/.test(source)) return 'eq5';
    if (/readelay|delay|echo/.test(source)) return 'delay';
    if (/reaverb|reverb|verb|room/.test(source)) return 'reverb';
    if (/realimit|limiter|maximizer|maximiser|brickwall/.test(source)) return 'limiter';
    return '';
  }

  function nativePluginChain(chunk, role = 'Other') {
    ...
    for (const rawLine of lines) {
      ...
      const bypass = line.match(/^BYPASS\s+(\d+)/i);
      if (bypass) {
        pendingBypass = Number(bypass[1]) !== 0;
        continue;
      }

      const open = line.match(/^<(VST|AU|CLAP|LV2|DX|JS|VIDEO_EFFECT)\s+"([^"]+)"/i);
      if (open) {
        const sourceName = cleanPluginName(open[2]);
        const nativeType = nativePluginType(sourceName);
        current = null;
        if (!nativeType) continue;
        current = {
          source_name: sourceName.slice(0,160),
          source_format: String(open[1]).toUpperCase().slice(0,20),
          source_preset: '',
          source_fxid: '',
          type: nativeType,
          enabled: !pendingBypass,
          params: nativePluginDefaults(nativeType, role),
          mapping: 'estimated',
          confidence: /rea(comp|eq|delay|verb|limit)/i.test(sourceName) ? 0.86 : 0.70
        };
        chain.push(current);
        pendingBypass = false;
        continue;
      ...
      const preset = line.match(/^PRESETNAME\s+"?([^"\r\n]+)"?/i);
      if (preset && current) {
        current.source_preset = text(preset[1]).slice(0,160);
        if (current.source_preset && !/^factory default|default|none$/i.test(current.source_preset)) {
          current.confidence = Math.max(current.confidence,0.78);
        }
        continue;
      }

      const fxid = line.match(/^FXID\s+(\{[^}]+\})/i);
      if (fxid && current) current.source_fxid = text(fxid[1]).slice(0,80);
    }
  ...
  function parseRppText(rppText, fileName = 'REAPER Project.rpp') {
    ...
    const masterFxStart = upperSource.indexOf('<MASTERFXLIST');
    if (masterFxStart >= 0) {
      const firstTrackStart = upperSource.indexOf('<TRACK', masterFxStart + 1);
      const masterFxEnd = firstTrackStart > masterFxStart
        ? firstTrackStart
        : source.length;
      result.master_plugin_chain = nativePluginChain(
        source.slice(masterFxStart, masterFxEnd),
        'Master'
      );
    }
    ...
    for (const part of parts) {
      ...
      const pluginChain = nativePluginChain(part, role);
