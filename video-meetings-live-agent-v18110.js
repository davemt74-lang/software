...
function ensurePane(){
  ...
  const start=node('button','','Start Agent');start.type='button';start.id='meetingLiveAgentStart';const stop=node('button','','Stop Agent');stop.type='button';stop.id='meetingLiveAgentStop';const status=node('span','','Loading live Agent…');status.id='meetingLiveAgentStatus';controls.append(start,stop,status);
...
function renderMessages(turns){const el=$('#meetingLiveAgentMessages');if(!el)return;el.innerHTML='';const rows=(Array.isArray(turns)?turns:[]).filter(t=>String(t?.question||'').trim()||String(t?.answer||'').trim());if(!rows.length){el.appendChild(node('div','meeting-agent-empty','Start the Agent, then ask it to summarize, clarify, challenge, or reason about the live meeting. HomeServer-only replies remain in this browser session instead of cloud history.'));return;}for(const turn of rows){const card=node('article','meeting-intelligence-item');const q=node('strong','',String(turn.question||''));const a=node('p','',String(turn.answer||''));card.append(q,a);const meta=[turn.private_ephemeral?'private · ephemeral':'',turn.route,turn.created_at].filter(Boolean).join(' · ');if(meta)card.appendChild(node('span','',meta));el.appendChild(card);}el.scrollTop=el.scrollHeight;}
...
async function ask(){if(busy||!state?.active)return;const input=$('#meetingLiveAgentInput');const question=String(input?.value||'').trim();if(!question)return;busy=true;render(state);try{const data=await request('ask',{question,client_turn_id:turnId()});if(input)input.value='';const next=data.state||{};if(data.turn?.private_ephemeral){const turns=(Array.isArray(next.turns)?next.turns:[]).filter(t=>String(t?.client_turn_id||'')!==String(data.turn.client_turn_id||''));turns.push(data.turn);next.turns=turns;}render(next);}catch(err){const s=$('#meetingLiveAgentStatus');if(s)s.textContent=err.message||'Live Agent could not answer.';}finally{busy=false;render(state);}}
