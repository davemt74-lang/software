...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    #sfListeningTranscriptNav{grid-row:2!important;position:fixed!important;z-index:10015!important;left:var(--sf-listening-sidebar-width)!important;right:0!important;bottom:var(--sf-listening-player-height)!important;width:auto!important;max-width:none!important;height:var(--sf-listening-nav-height)!important;min-height:var(--sf-listening-nav-height)!important;max-height:var(--sf-listening-nav-height)!important;margin:0!important;padding:6px 12px!important;display:flex!important;align-items:center!important;justify-content:flex-start!important;flex-wrap:nowrap!important;gap:6px!important;overflow-x:auto!important;overflow-y:hidden!important;white-space:nowrap!important;border:0!important;border-top:1px solid #e3e3e3!important;border-bottom:1px solid #e3e3e3!important;border-radius:0!important;background:#fff!important;box-shadow:none!important}
    ...
    .sf-listening-workspace-listening-player{position:fixed!important;z-index:10020!important;left:0!important;right:0!important;bottom:0!important;width:100vw!important;height:var(--sf-listening-player-height)!important;min-height:var(--sf-listening-player-height)!important;max-height:var(--sf-listening-player-height)!important;margin:0!important;padding:8px 16px 8px calc(var(--sf-listening-sidebar-width) + 16px)!important;display:flex!important;align-items:center!important;justify-content:flex-start!important;flex-direction:row!important;flex-wrap:nowrap!important;gap:12px!important;overflow-x:auto!important;overflow-y:hidden!important;white-space:nowrap!important;border-top:1px solid #ddd!important;background:rgba(255,255,255,.98)!important;box-shadow:0 -8px 24px rgba(0,0,0,.08)!important;backdrop-filter:blur(12px)}
    .sf-listening-workspace-capture-actions.sf-listening-footer-left{display:flex!important;align-items:center!important;justify-content:flex-start!important;flex:0 0 auto!important;flex-wrap:nowrap!important;gap:7px!important;overflow:visible!important;white-space:nowrap!important}
    .sf-listening-workspace-capture-actions.sf-listening-footer-left .sf-listening-workspace-live-status{display:flex!important;align-items:center!important;flex:0 0 auto!important;gap:6px!important;margin-left:4px!important;white-space:nowrap!important}
    .sf-listening-footer-right{display:flex!important;align-items:center!important;flex:0 0 auto!important;flex-wrap:nowrap!important;gap:7px!important;margin-left:auto!important;white-space:nowrap!important}
    .sf-listening-footer-right [data-listening-workspace-save],.sf-listening-footer-right [data-listening-start]{flex:0 0 auto!important}
    ...
    .sf-listening-ai-apps-head-secondary{margin-top:12px;padding-top:10px;border-top:1px solid #ececec}
    ...
    .sf-listening-ai-presets{display:flex;gap:5px;overflow-x:auto;padding-bottom:7px}
    .sf-listening-ai-presets button{flex:0 0 auto;min-height:28px;padding:5px 8px;border:1px solid #ddd;border-radius:7px;background:#fff;color:#666;font-size:9px;font-weight:800;cursor:pointer}
    .sf-listening-ai-presets button:hover,.sf-listening-ai-presets button.active{border-color:#aaa;background:#f2f2f2;color:#171717}
    .sf-listening-ai-workflow-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin:2px 0 7px}
    .sf-listening-ai-workflow-grid label{display:grid;gap:4px;color:#777;font-size:8px;font-weight:800;text-transform:uppercase;letter-spacing:.04em}
    .sf-listening-ai-workflow-grid select,.sf-listening-ai-workflow-grid input{width:100%;min-width:0;height:32px;padding:6px 8px;border:1px solid #ddd;border-radius:7px;background:#fff;color:#222;font:600 10px/1.2 system-ui;text-transform:none;letter-spacing:0}
    .sf-listening-ai-workflow-grid .sf-listening-ai-focus{grid-column:1/-1}
    .sf-listening-ai-run-plan{padding:7px 8px;border:1px solid #e5e5e5;border-radius:7px;background:#fafafa;color:#666;font-size:9px;line-height:1.35}
    ...
    .sf-listening-ai-tabs button.error{color:#9a3838}
    ...
    @media(max-width:900px) and (min-width:721px){:root{--sf-listening-sidebar-width:238px;--sf-listening-player-height:60px}}
    @media(max-width:720px){
      ...
      .sf-listening-workspace-sidebar{position:fixed!important;z-index:10060!important;inset:0 auto 0 0!important;display:grid!important;grid-template-rows:auto auto auto minmax(0,1fr)!important;width:min(86vw,320px)!important;max-width:320px!important;height:100dvh!important;min-height:0!important;overflow:hidden!important;border:0!important;border-right:1px solid #ddd!important;background:#fafafa!important;box-shadow:18px 0 45px rgba(0,0,0,.16)!important;transform:translateX(-104%)!important;transition:transform .2s ease!important}
      ...
      .sf-listening-ai-workflow-grid{grid-template-columns:1fr}
      .sf-listening-ai-workflow-grid .sf-listening-ai-focus{grid-column:auto}
...
<script>
  ...
  (() => {
    ...
    let wired = false;
    function closeMobileMenu(){document.body.classList.remove('sf-listening-mobile-menu-open');const toggle=document.querySelector('[data-listening-mobile-menu-toggle]');if(toggle)toggle.setAttribute('aria-expanded','false')}
    function toggleMobileMenu(){const open=!document.body.classList.contains('sf-listening-mobile-menu-open');document.body.classList.toggle('sf-listening-mobile-menu-open',open);const toggle=document.querySelector('[data-listening-mobile-menu-toggle]');if(toggle)toggle.setAttribute('aria-expanded',open?'true':'false')}
    function ensureUserMenu(editorTop){
      const toolbar=editorTop?.querySelector('.sf-listening-workspace-toolbar');const aiButton=toolbar?.querySelector('[data-listening-ai-toggle]');const userId=Math.max(0,Number(listeningConfig.userId||0));if(!toolbar||!aiButton||!userId)return false;if(toolbar.querySelector('[data-listening-user-menu]'))return true;
      const menu=document.createElement('div');menu.className='sf-listening-user-menu';menu.dataset.listeningUserMenu='1';menu.innerHTML=`<button type="button" class="sf-listening-user-menu-button" data-listening-user-menu-toggle aria-expanded="false" aria-controls="sfListeningUserMenuDropdown" aria-label="Open user menu"><span class="sf-listening-user-avatar"><span data-listening-user-avatar-fallback aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="3.5"></circle><path d="M5.5 19c.8-3.7 3-5.5 6.5-5.5s5.7 1.8 6.5 5.5"></path></svg></span><img data-listening-user-avatar hidden alt=""></span></button><div class="sf-listening-user-menu-dropdown" id="sfListeningUserMenuDropdown" data-listening-user-menu-dropdown hidden></div>`;
      const dropdown=menu.querySelector('[data-listening-user-menu-dropdown]');const menuLinks=Array.isArray(listeningConfig.userMenuLinks)?listeningConfig.userMenuLinks:[];menuLinks.forEach(item=>{if(!item||!item.url||!item.label||!dropdown)return;const link=document.createElement('a');link.href=String(item.url);if(item.danger)link.classList.add('logout');const label=document.createElement('span');label.textContent=String(item.label);const arrow=document.createElement('span');arrow.textContent='↗';link.append(label,arrow);dropdown.appendChild(link)});aiButton.insertAdjacentElement('afterend',menu);
      const button=menu.querySelector('[data-listening-user-menu-toggle]');const image=menu.querySelector('[data-listening-user-avatar]');const fallback=menu.querySelector('[data-listening-user-avatar-fallback]');const close=()=>{if(!button||!dropdown)return;dropdown.hidden=true;button.setAttribute('aria-expanded','false')};
      if(image){image.addEventListener('load',()=>{image.hidden=false;if(fallback)fallback.hidden=true},{once:true});image.addEventListener('error',()=>{image.hidden=true;if(fallback)fallback.hidden=false},{once:true});image.src=`avatar.php?user=${encodeURIComponent(String(userId))}`}
      button?.addEventListener('click',event=>{event.stopPropagation();const opening=Boolean(dropdown?.hidden);if(dropdown)dropdown.hidden=!opening;button.setAttribute('aria-expanded',opening?'true':'false')});dropdown?.addEventListener('click',event=>event.stopPropagation());document.addEventListener('click',event=>{if(!menu.contains(event.target))close()});document.addEventListener('keydown',event=>{if(event.key==='Escape')close()});return true
    }
    function composeLayout(){
      const workspace=document.querySelector('.sf-transcript-workspace');const sidebar=workspace?.querySelector('.sf-listening-workspace-sidebar');const sideHead=sidebar?.querySelector('.sf-listening-workspace-side-head');const editorTop=workspace?.querySelector('.sf-listening-workspace-editor-top');const player=workspace?.querySelector('.sf-listening-workspace-listening-player');const capture=player?.querySelector('.sf-listening-workspace-capture-actions');const liveStatus=player?.querySelector('.sf-listening-workspace-live-status');const save=workspace?.querySelector('[data-listening-workspace-save]');const start=workspace?.querySelector('[data-listening-start]');const exit=workspace?.querySelector('[data-listening-workspace-exit]');if(!workspace||!sidebar||!sideHead||!editorTop||!player||!capture||!save||!start)return false;
      if(!sidebar.id)sidebar.id='sfListeningMobileLibrary';const newRecording=sideHead.querySelector('[data-listening-workspace-new]');if(exit&&exit.parentElement!==sideHead)sideHead.insertBefore(exit,newRecording||null);ensureUserMenu(editorTop);capture.classList.add('sf-listening-footer-left');const pause=workspace.querySelector('[data-listening-workspace-pause]');const stop=workspace.querySelector('[data-listening-stop]');const record=workspace.querySelector('[data-listening-record]');const marker=workspace.querySelector('[data-listening-marker]');const note=workspace.querySelector('[data-listening-note]');[pause,stop,record,marker,note,liveStatus].forEach(node=>{if(node)capture.appendChild(node)});
      let right=player.querySelector('.sf-listening-footer-right');if(!right){right=document.createElement('div');right.className='sf-listening-footer-right';player.appendChild(right)}if(save.parentElement!==right)right.appendChild(save);if(start.parentElement!==right)right.appendChild(start);
      let mobileToggle=editorTop.querySelector('[data-listening-mobile-menu-toggle]');if(!mobileToggle){mobileToggle=document.createElement('button');mobileToggle.type='button';mobileToggle.className='sf-listening-mobile-menu-toggle';mobileToggle.dataset.listeningMobileMenuToggle='1';mobileToggle.setAttribute('aria-label','Open transcript library');mobileToggle.setAttribute('aria-controls',sidebar.id);mobileToggle.setAttribute('aria-expanded','false');mobileToggle.textContent='☰';editorTop.prepend(mobileToggle)}
      let shade=document.querySelector('[data-listening-mobile-menu-shade]');if(!shade){shade=document.createElement('button');shade.type='button';shade.className='sf-listening-mobile-menu-shade';shade.dataset.listeningMobileMenuShade='1';shade.setAttribute('aria-label','Close transcript library');document.body.appendChild(shade)}
      if(!wired){wired=true;document.addEventListener('click',event=>{if(event.target.closest?.('[data-listening-mobile-menu-toggle]')){event.preventDefault();toggleMobileMenu();return}if(event.target.closest?.('[data-listening-mobile-menu-shade]')){event.preventDefault();closeMobileMenu();return}if(!mobileQuery.matches)return;if(event.target.closest?.('[data-listening-workspace-file-open],[data-listening-workspace-folder],[data-listening-workspace-new],[data-listening-workspace-create]'))closeMobileMenu()});document.addEventListener('keydown',event=>{if(event.key==='Escape')closeMobileMenu()});mobileQuery.addEventListener?.('change',event=>{if(!event.matches)closeMobileMenu()})}
      return true
    }
    function bootLayout(){if(composeLayout())return;let attempts=0;const timer=setInterval(()=>{attempts+=1;if(composeLayout()||attempts>=40)clearInterval(timer)},100)}
