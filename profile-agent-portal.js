...
function humanActivityDetail(e){
  if(e?.target_title){const label=esc(e.target_title);return e.target_url?`${label} · <a href="${esc(e.target_url)}" target="_blank" rel="noopener">Open ↗</a>`:label;}
...
function radarSessions(){
  const rows=[];
  if(radarMatches('human')){
    (state?.visits||[]).forEach(v=>rows.push({date:v.last_seen_at,html:`<div class="profile-agent-radar-session"><div><strong>${esc(v.visitor_label||'Visitor')}</strong><small>Human · ${v.signed_in?'member':'guest'}</small></div><code>Public profile</code><span>${Number(v.view_count||0)} view${Number(v.view_count||0)===1?'':'s'} · ${Number(v.request_count||0)} req.</span><span>${esc(relative(v.first_seen_at))} → ${esc(relative(v.last_seen_at))}</span></div>`}));
  }
