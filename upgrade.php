...
function vp3_upgrade_complete(): bool
{
    return access_schema_ready()
        ...
        && agent_commerce_appointment_adapter_ready_v800()
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf()) {
        ...
        try {
            ...
            agent_commerce_appointment_adapter_ensure_v800($pdo);
            ...
            if ($complete) {
                flash('notice', 'VP3 database upgrade complete: subscriptions, composable product entitlements, canonical plugin lifecycle, canonical human messaging, durable Agent retirement, Agent-scoped Brain memory, native Agent Scheduling, external calendar synchronization, Team Scheduling, Appointment Lifecycle + Automation, Commerce + Paid Scheduling, canonical Agent runtime routing and route-attributed AI accounting, social relationships, Team invitation/lifecycle collaboration, workspace-owned Music resources, Knowledge, Profile Agent, HomeServer Agent continuity, Agent Radar, CRM, transcriptions, and Music/Studio capabilities are ready.');
...
<main class="vp3-auth-shell">
  ...
  <section class="vp3-auth-form-side">
    <div class="vp3-auth-card">
      ...
      <?php else: ?>
        <p class="vp3-auth-intro">Run the current schema upgrade while preserving existing content and access. Existing accounts, package assignments, team memberships, token balances, music content, plugin preferences, Agent identities, Agent-scoped Brain memories, schedules, canonical bookings, lifecycle history, intake answers, automation deliveries, payment-provider connections, paid-booking lineage, commerce products/offers/orders/payments/refunds/fulfillment, calendar connections, Team scheduling pools and conversations are preserved, including Team membership history, legacy Team direct messages and existing add-on grants.</p>
