...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if(!verify_csrf())$error='Session expired. Please try again.';
        ...
        ensure_access_schema();permission_v105_seed_playlist_permission();midi_v217_ensure_schema();
        ...
        if($complete){flash('notice','Stonefellow database upgrade complete: canonical profiles, Profile Agent conversations, proactive attention queue, user-owned agents, shared knowledge audit, Voice Profile, Artist Listening, CRM, music, chat and Studio features are ready.');redirect(url('/admin/users.php'));}
