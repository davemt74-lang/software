...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if(!verify_csrf())$error='Session expired. Please try again.';
        ...
        if($complete){flash('notice','Stonefellow database upgrade complete: personal Agent Brain, personal Knowledge, Profile Agent/Profile Chat, Voice Profile, transcriptions, shared/system knowledge, canonical profiles and Studio features are ready.');redirect(url('/admin/users.php'));}
...
<?php if($complete): ?><p style="color:#cfc2b1;line-height:1.7">The current schema is installed, including role-configurable personal Agent Brain, private per-user Knowledge, Profile Agent/Profile Chat, voice identity, transcriptions, shared/system knowledge and retrieval transparency.</p><a class="btn primary" href="<?= e(url('/admin/users.php')) ?>">Manage Users</a>
<?php else: ?><p style="color:#aaa095;line-height:1.7">This upgrade separates personal account capabilities from system/shared data while preserving existing content and permissions. Existing knowledge remains in the system library; personal knowledge is explicitly owner-scoped.</p><?php if($error): ?><p style="color:#e78a7d"><?= e($error) ?></p><?php endif; ?><form method="post"><?= csrf_field() ?><button class="form-submit" type="submit">Run Upgrade</button></form><?php endif; ?>
