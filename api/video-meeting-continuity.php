...
try{
    if($action==='state'){echo json_encode(['ok'=>true,'continuity'=>video_meeting_continuity_state_v18210($pdo,$meeting,$userId)],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);exit;}
    if($action==='carry_forward'){echo json_encode(['ok'=>true,'continuity'=>video_meeting_continuity_carry_forward_v18210($pdo,$meeting,$userId,(int)($input['source_agenda_item_id']??0))],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);exit;}
    if($action==='set_status'){echo json_encode(['ok'=>true,'continuity'=>video_meeting_continuity_set_status_v18210($pdo,$meeting,$userId,(int)($input['continuity_link_id']??0),(string)($input['status']??''))],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);exit;}
