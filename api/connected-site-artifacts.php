...
$pdo=db();try{
 $auth=vp3_connected_site_auth_v100($pdo,'meetings.transcripts.read');$uid=(int)$auth['user_id'];$id=strtolower(trim((string)($_GET['id']??'')));
 if($id===''){$rows=[];foreach(video_meeting_recent_for_user_v1800($pdo,$uid,80) as $m)$rows[]=vp3_connected_site_meeting_descriptor_v100($pdo,$m,$uid,$auth['scopes']);echo json_encode(['ok'=>true,'data'=>['artifacts'=>$rows]],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);exit;}
 $m=vp3_connected_site_meeting_access_v100($pdo,$uid,$id);if(!$m){http_response_code(404);throw new RuntimeException('VP3 meeting is unavailable.');}$descriptor=vp3_connected_site_meeting_descriptor_v100($pdo,$m,$uid,$auth['scopes']);$transcript=vp3_connected_site_meeting_transcript_v100($pdo,$m);$summary=in_array('meetings.intelligence.read',$auth['scopes'],true)?vp3_connected_site_meeting_summary_v100($pdo,$m,$uid):null;
