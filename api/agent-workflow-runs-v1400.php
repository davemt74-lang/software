...
try{
    ...
    $action=trim((string)($input['action']??''));$run=null;$durable=agent_job_engine_schema_ready_v1900($pdo);
    ...
    if($action==='create_from_brain'){
        ...
        if($durable){$id=(int)($input['run_id']??0);$pdo->prepare("UPDATE agent_workflow_runs SET next_attempt_at=UTC_TIMESTAMP(),progress_message='Approved and ready' WHERE id=? AND owner_user_id=?")->execute([$id,(int)$user['id']]);$row=agent_workflow_row_v1400($pdo,(int)$user['id'],$id);if($row)$run=agent_job_public_run_v1900($pdo,$row,true);}
    }elseif($action==='cancel'){
        $run=$durable?agent_job_cancel_v1900($pdo,$user,(int)($input['run_id']??0)):agent_workflow_cancel_v1400($pdo,$user,(int)($input['run_id']??0));
    }elseif($action==='retry'){
        $run=$durable?agent_job_retry_v1900($pdo,$user,(int)($input['run_id']??0)):agent_workflow_retry_v1400($pdo,$user,(int)($input['run_id']??0));
    }else{
        throw new RuntimeException('Unknown workflow action.');
    }
    echo json_encode(['ok'=>true,'run'=>$run,'build'=>$durable?VP3_AGENT_JOB_ENGINE_V1900:VP3_AGENT_WORKFLOW_RUNS_V1400],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
}catch(Throwable $e){
