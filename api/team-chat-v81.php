...
function team_chat_json(array $payload, int $status = 200): never
{
...
function team_chat_avatar_url(string $path): string
{
    $path = trim($path);
    if ($path === '') {
        return '';
...
function team_chat_user_payload(array $row): array
{
    return [
        'id'=>(int)$row['id'],
        ...
        'avatar'=>team_chat_avatar_url((string)($row['avatar_path'] ?? '')),
...
function team_chat_target(PDO $pdo, int $userId): ?array
{
    if ($userId < 1) {
        return null;
    }
    ...
    $stmt = $pdo->prepare(
        "SELECT id,display_name,role,avatar_path,is_active
         ...
         WHERE id=?
           AND is_active=1
           AND role IN ('manager','producer','supervisor')
         LIMIT 1"
    );
    $stmt->execute([$userId]);
    $row = $stmt->fetch();
...
function team_chat_touch(PDO $pdo, int $userId, string $page, string $context): void
{
    $page = preg_replace('/[^a-z0-9_-]/i', '', $page) ?: 'workspace';
    $page = substr($page, 0, 60);
    $context = trim($context);
    if (mb_strlen($context) > 190) {
        $context = mb_substr($context, 0, 190);
    }
...
function team_chat_message_payload(array $row): array
{
    return [
        'id'=>(int)$row['id'],
        ...
        'sender'=>[
            ...
            'avatar'=>team_chat_avatar_url((string)($row['sender_avatar'] ?? '')),
        ],
        'recipient'=>[
            ...
            'avatar'=>team_chat_avatar_url((string)($row['recipient_avatar'] ?? '')),
        ],
    ];
}
...
$user = current_user();
if (!$user) {
    team_chat_json(['ok'=>false,'error'=>'login_required'], 401);
}

if (!team_chat_role_allowed($user)) {
    team_chat_json(['ok'=>false,'error'=>'forbidden_role'], 403);
}

if (!table_exists('team_user_presence') || !table_exists('team_direct_messages')) {
    team_chat_json(['ok'=>false,'error'=>'upgrade_required'], 503);
}

$pdo = db();
if (!$pdo) {
    team_chat_json(['ok'=>false,'error'=>'database_unavailable'], 503);
}
...
try {
    team_chat_touch($pdo, $currentId, $page, $context);
    ...
    if ($action === 'poll') {
        ...
        $onlineStmt = $pdo->prepare(
            "SELECT
             ...
             WHERE u.id<>?
               AND u.is_active=1
               AND u.role IN ('manager','producer','supervisor')
        ...
        $online = array_map('team_chat_user_payload', $onlineStmt->fetchAll());
        ...
        if ($since < 1) {
            ...
            $messages = array_map('team_chat_message_payload', $rows);
            $cursor = $since;
            foreach ($rows as $row) {
    ...
    if ($action === 'history') {
        ...
        $target = team_chat_target($pdo, $targetId);

        if (!$target || $targetId === $currentId) {
            team_chat_json(['ok'=>false,'error'=>'user_not_available'], 404);
        }
        ...
        $messages = array_map('team_chat_message_payload', $historyStmt->fetchAll());
        ...
        team_chat_json([
            'ok'=>true,
            'user'=>team_chat_user_payload($target),
            'messages'=>$messages,
        ]);
    }
    ...
    if ($action === 'send') {
        if (!verify_csrf()) {
            team_chat_json(['ok'=>false,'error'=>'csrf'], 419);
        }
        ...
        if ($message === '') {
            team_chat_json(['ok'=>false,'error'=>'empty_message'], 422);
        }

        if (mb_strlen($message) > 2000) {
            team_chat_json(['ok'=>false,'error'=>'message_too_long'], 422);
        }
        ...
        team_chat_json([
            'ok'=>true,
            'message'=>team_chat_message_payload($row),
        ]);
    }
    ...
    if ($action === 'read') {
        ...
        team_chat_json(['ok'=>true]);
    }

    team_chat_json(['ok'=>false,'error'=>'unknown_action'], 400);
} catch (Throwable $e) {
    error_log('Stonefellow team chat error: ' . $e->getMessage());
    team_chat_json(['ok'=>false,'error'=>'team_chat_failed'], 500);
}
