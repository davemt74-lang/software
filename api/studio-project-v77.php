...
function studio_v65_require_catalog_manager(): void
{
    if (!has_permission('tracks.manage')) {
        studio_v47_error(
            'This action is limited to track managers.',
            403
        );
    }
}
...
function studio_v159_role(string $value): string
{
    $clean = stem_lower(trim($value));
    $map = [
        'vocal'=>'Vocal','vocals'=>'Vocal','voice'=>'Vocal',
        'drum'=>'Drums','drums'=>'Drums','percussion'=>'Percussion',
        'bass'=>'Bass','guitar'=>'Guitar','keys'=>'Keys',
        'keyboard'=>'Keys','synth'=>'Synth','audio'=>'Other','other'=>'Other',
    ];
    if (isset($map[$clean])) return $map[$clean];
    $custom = trim((string)preg_replace('/[^\pL\pN _-]+/u','',$value));
    return $custom !== '' ? studio_v53_cut(mb_convert_case($custom,MB_CASE_TITLE,'UTF-8'),32) : 'Other';
}

function studio_v159_measure_seconds(
    float $tempo,
    string $signature
): float {
    $numerator = 4;
    $denominator = 4;
    if (preg_match('/^(\d{1,2})\/(\d{1,2})$/',trim($signature),$match)) {
        $numerator = max(1,(int)$match[1]);
        $denominator = max(1,(int)$match[2]);
    }
    return $numerator * (4.0 / $denominator) * 60.0 / max(40.0,min(300.0,$tempo));
}

function studio_v159_create_empty_track(
    PDO $pdo,
    int $trackId,
    int $projectId,
    int $userId,
    ...
    $name = studio_v53_cut(trim((string)($spec['track_name'] ?? 'Audio Track')),190);
    if ($name === '') $name = 'Audio Track';
    $role = studio_v159_role((string)($spec['stem_role'] ?? 'Other'));
    $sampleRate = max(8000,min(192000,(int)($spec['sample_rate'] ?? 48000)));
    $sessionTempo = max(40.0,min(300.0,(float)($spec['session_tempo'] ?? 120)));
    $startOffset = max(0.0,(float)($spec['start_offset'] ?? 0));
    $inputLabel = studio_v53_cut(trim((string)($spec['input_label'] ?? '')),190);
    $inputChannel = max(1,min(64,(int)($spec['input_channel'] ?? 1)));
    $channels = 2;
    $durationSeconds = 0.05;
    $frames = max(1,(int)round($sampleRate * $durationSeconds));
    $pcmBytes = $frames * $channels * 2;
    $token = bin2hex(random_bytes(8));
    $relativeDir = '/uploads/stems/track-' . $trackId . '/empty-' . $token;
    $absoluteDir = STONEFELLOW_ROOT . $relativeDir;
    if (!is_dir($absoluteDir) && !mkdir($absoluteDir,0700,true) && !is_dir($absoluteDir)) {
        throw new RuntimeException('Could not create recording-track storage.');
    }
    $fileName = stem_clean_filename($name);
    if ($fileName === '') $fileName = 'Audio Track';
    $fileName .= '.wav';
    $absolutePath = $absoluteDir . '/' . $fileName;
    $relativePath = $relativeDir . '/' . $fileName;
    $wav = studio_v53_wav_header($pcmBytes,$sampleRate,$channels,16) . str_repeat("\0",$pcmBytes);
    if (file_put_contents($absolutePath,$wav,LOCK_EX) === false) {
        @unlink($absolutePath);@rmdir($absoluteDir);
        throw new RuntimeException('Could not create the empty recording track.');
    }
    @chmod($absolutePath,0600);
    $createdPaths[] = [$absolutePath,$absoluteDir];
    ...
    $summary = 'Empty recording track · Recorded tempo: '
        . rtrim(rtrim(number_format($sessionTempo,2,'.',''),'0'),'.') . ' BPM';
    if ($inputLabel !== '') $summary .= ' · Recording input: ' . str_replace('·','-', $inputLabel);
    $summary .= ' · Recording channel: ' . $inputChannel;

    $insert = $pdo->prepare(
        'INSERT INTO track_stems
         (track_id,project_id,stem_name,stem_role,source_track_name,file_name,file_path,channels,sample_rate,bit_depth,duration_seconds,start_offset_seconds,rpp_track_guid,rpp_volume,rpp_pan,rpp_fx_summary,sort_order,is_active)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)'
    );
    $insert->execute([
        $trackId,$projectId,$name,$role,$name,$fileName,$relativePath,$channels,$sampleRate,16,
        $durationSeconds,$startOffset,'',1,0,$summary,$sort,
    ]);
    return [
        'stem_id'=>(int)$pdo->lastInsertId(),
        'track_name'=>$name,
        'stem_role'=>$role,
...
try {
    if ($action === 'create_project') {
        studio_v65_require_catalog_manager();
    ...
    if ($action === 'update_project_duration') {
        $trackId = (int)($_POST['track_id'] ?? 0);
        studio_v47_track($trackId);
        $userId = studio_v47_user_id();
        if ($userId < 1) throw new RuntimeException('Sign in before setting song duration.');
        studio_v47_ensure_project($trackId,$userId);
        if (!column_exists('track_projects','duration_measures')) {
            throw new RuntimeException('Run the cumulative database upgrade before setting song duration.');
        }
        $measures = max(1,min(4096,(int)($_POST['duration_measures'] ?? 0)));
        $stmt = $pdo->prepare(
            "SELECT COALESCE(NULLIF(p.tempo_bpm,0),NULLIF(t.tempo_bpm,0),120) tempo,
                    COALESCE(NULLIF(p.time_signature,''),'4/4') signature
             FROM tracks t LEFT JOIN track_projects p ON p.track_id=t.id
             WHERE t.id=? LIMIT 1"
        );
        $stmt->execute([$trackId]);
        $row = $stmt->fetch() ?: ['tempo'=>120,'signature'=>'4/4'];
        $update = $pdo->prepare('UPDATE track_projects SET duration_measures=?,updated_at=NOW() WHERE track_id=?');
        $update->execute([$measures,$trackId]);
        $pdo->prepare('UPDATE tracks SET updated_at=NOW() WHERE id=?')->execute([$trackId]);
        studio_v47_json([
            'ok'=>true,'track_id'=>$trackId,'duration_measures'=>$measures,
            'duration_seconds'=>$measures * studio_v159_measure_seconds((float)$row['tempo'],(string)$row['signature']),
            'tempo_bpm'=>(float)$row['tempo'],'time_signature'=>(string)$row['signature'],
        ]);
    }
    ...
    if ($action === 'rename_project') {
        ...
        studio_v47_track($trackId);
        $userId = studio_v47_user_id();
        if ($userId < 1) throw new RuntimeException('Sign in before renaming a project.');
        studio_v47_ensure_project($trackId,$userId);
        $name = studio_v53_cut(trim((string)($_POST['project_name'] ?? '')),190);
        if ($name === '') throw new RuntimeException('Project name is required.');
        $pdo->beginTransaction();
        try {
            $pdo->prepare('UPDATE tracks SET title=?,updated_at=NOW() WHERE id=?')->execute([$name,$trackId]);
            $pdo->prepare('UPDATE track_projects SET project_name=?,updated_at=NOW() WHERE track_id=?')->execute([$name,$trackId]);
            $pdo->commit();
        } catch (Throwable $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            throw $e;
        }
        studio_v47_json(['ok'=>true,'track_id'=>$trackId,'project_name'=>$name]);
    }

    if ($action === 'create_empty_tracks') {
        $trackId = (int)($_POST['track_id'] ?? 0);
        studio_v47_track($trackId);
        $userId = studio_v47_user_id();
        if ($userId < 1) throw new RuntimeException('Sign in before creating recording tracks.');
        $specs = json_decode((string)($_POST['tracks_json'] ?? '[]'),true);
        if (!is_array($specs) || count($specs) < 1 || count($specs) > 64) {
            throw new RuntimeException('Create between 1 and 64 recording tracks at a time.');
        }
        $project = studio_v47_ensure_project($trackId,$userId);
        $createdPaths = [];$created = [];
        $pdo->beginTransaction();
        try {
            foreach ($specs as $spec) {
                if (!is_array($spec)) throw new RuntimeException('Invalid recording-track request.');
                $created[] = studio_v159_create_empty_track($pdo,$trackId,(int)$project['id'],$userId,$spec,$createdPaths);
            }
            $pdo->prepare('UPDATE tracks SET owner_user_id=COALESCE(owner_user_id,?),updated_at=NOW() WHERE id=?')->execute([$userId,$trackId]);
            studio_v47_refresh_track_duration($trackId);
            $pdo->commit();
        } catch (Throwable $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            foreach ($createdPaths as [$file,$dir]) {@unlink($file);@rmdir($dir);}
            throw $e;
        }
        studio_v47_json([
            'ok'=>true,'track_id'=>$trackId,'created'=>$created,'count'=>count($created),
            'redirect'=>url('/admin/stems.php?track=' . $trackId),
            'message'=>count($created) . ' empty recording tracks created.',
        ]);
    }

    if ($action === 'update_stem') {
        $trackId =
        ...
        $stmt = $pdo->prepare(
            "UPDATE track_stems
             SET
                ...
                source_track_name=
                  CASE
                    WHEN source_track_name=stem_name
                      OR source_track_name=''
                    THEN ?
                    ELSE source_track_name
                  END,
                updated_at=NOW()
             WHERE id=?
               AND track_id=?
        ...
        studio_v47_json([
            ...
            'stem_name'=>$name,
            'stem_role'=>$role,
            'message'=>'Track details saved.',
        ]);
    }

    if ($action === 'create_empty_track') {
        $trackId =
            (int)($_POST['track_id'] ?? 0);
        ...
        if ($userId < 1) {
            throw new RuntimeException(
                'Sign in before creating a recording track.'
            );
        }
        ...
        $role = studio_v159_role((string)($_POST['stem_role'] ?? 'Other'));
        $inputLabel = studio_v53_cut(trim((string)($_POST['input_label'] ?? '')),190);
        $inputChannel = max(1,min(64,(int)($_POST['input_channel'] ?? 1)));
        ...
        if (
            ...
            throw new RuntimeException(
                'Could not create recording-track storage.'
            );
        }

        $fileName =
            stem_clean_filename($name);
        ...
        if (
            ...
            throw new RuntimeException(
                'Could not create the empty recording track.'
            );
        }
        ...
        $summary =
            ...
            . ' BPM'
            . ($inputLabel !== '' ? ' · Recording input: ' . str_replace('·','-',$inputLabel) : '')
            . ' · Recording channel: ' . $inputChannel;
        ...
        studio_v47_json([
            ...
            'input_label'=>$inputLabel,
            'input_channel'=>$inputChannel,
            'message'=>'Empty recording track created.',
        ]);
    }
    ...
    if ($action === 'recording_start') {
        ...
        $targetStemId =
            max(
                0,
                (int)(
                    $_POST['target_stem_id'] ??
                    0
                )
            );
        $createNewTake = !empty($_POST['create_new']);
        $captureKind = studio_v53_cut(
            trim((string)($_POST['capture_kind'] ?? 'input')),
            32
        );
        $requestedRole = studio_v53_cut(
            trim((string)($_POST['stem_role'] ?? 'Other')),
            32
        );
        $allowedCaptureRoles = [
            'Vocal','Drums','Percussion','Bass','Guitar','Keys','Synth','Other'
        ];
        if (!in_array($requestedRole,$allowedCaptureRoles,true)) {
            $requestedRole = 'Other';
        }

        if ($targetStemId > 0) {
            $targetStmt = $pdo->prepare(
                'SELECT id,rpp_fx_summary
                 FROM track_stems
                 WHERE id=?
                   AND track_id=?
                   AND is_active=1
        ...
        if ($targetStemId < 1 && !$createNewTake) {
            throw new RuntimeException(
                'Arm a track before recording.'
            );
        }
        ...
        $state = [
            ...
            'create_new'=>$createNewTake ? 1 : 0,
            'capture_kind'=>$captureKind,
            'stem_role'=>$requestedRole,
        ...
        if (function_exists('agent_tool_log')) {
            agent_tool_log(
                current_user() ?: [],
                'stem_studio.live_recording_start',
    ...
    if ($action === 'recording_finish') {
        ...
        $createNewTake = !empty($state['create_new']);

        if (!$replaceTarget && !$createNewTake) {
            throw new RuntimeException(
                'The armed recording track is no longer available.'
            );
        }
        ...
        try {
            ...
            $captureRole = (string)($state['stem_role'] ?? '');
            if (!in_array($captureRole,['Vocal','Drums','Percussion','Bass','Guitar','Keys','Synth','Other'],true)) {
                $captureRole = stem_role_from_metadata($trackName,$summary);
            }

            if ($createNewTake) {
                $insertStem = $pdo->prepare(
                    'INSERT INTO track_stems
                     (
                        track_id,project_id,stem_name,stem_role,source_track_name,
                        file_name,file_path,channels,sample_rate,bit_depth,
                        duration_seconds,start_offset_seconds,rpp_track_guid,
                        rpp_volume,rpp_pan,rpp_fx_summary,sort_order,is_active
                     )
                     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)'
                );
                $insertStem->execute([
                    $trackId,$projectId,$trackName,$captureRole,$trackName,
                    $fileName,$relativePath,$channels,$sampleRate,16,
                    $durationSeconds,(float)$state['start_offset'],'',1,0,
                    $summary,$sort,
                ]);
                $stemId = (int)$pdo->lastInsertId();
            } else {
                $updateStem = $pdo->prepare(
                    'UPDATE track_stems
                     SET
                        stem_name=?,stem_role=?,source_track_name=?,file_name=?,
        ...
        if (function_exists('agent_tool_log')) {
            agent_tool_log(
                current_user() ?: [],
                'stem_studio.live_recording_finish',
                (string)$state['track_name'],
                'saved',
                [
                    'track_id'=>$trackId,
                    'stem_id'=>$stemId,
                    'capture_kind'=>(string)($state['capture_kind'] ?? 'input'),
                    'duration'=>$durationSeconds,
                    'start_offset'=>(float)$state['start_offset'],
                ]
            );
        }
        ...
        studio_v47_json([
            ...
            'media_url'=>
                url(
                    '/stem-media-v34.php?id='
                    . $stemId
                ),
            'message'=>$createNewTake ? 'Live recording saved as a new Studio track.' : 'Recording saved to the armed Studio track.',
        ]);
    }
    ...
    if ($action === 'add_library_stem') {
        ...
        $sourceTrack = studio_v47_track(
            (int)$source['track_id']
        );

        if (!can_manage_track_production($sourceTrack)) {
            studio_v47_error(
                'The selected library stem is not shared with your account.',
                403
            );
        }
