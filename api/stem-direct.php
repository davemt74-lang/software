...
try {
    ...
    if ($action === 'init') {
        ...
        if (!is_array($files) || count($files) > 96) {
            throw new RuntimeException('Select up to 96 MP3 stem files.');
        }

        if (!$files && !$hasRpp) {
            throw new RuntimeException('Select at least one MP3 stem or a REAPER .rpp project file.');
        }
        ...
        foreach ($files as $index=>$file) {
            ...
            if (
                $name === '' ||
                !str_ends_with(mb_strtolower($name), '.mp3') ||
                $size < 1
            ) {
                throw new RuntimeException('Direct upload accepts MP3 stems only.');
            }
        ...
        if ($totalBytes > stem_max_package_bytes()) {
            throw new RuntimeException('The selected MP3 stems exceed the configured upload limit.');
        }
        ...
        if (is_numeric($free) && (float)$free < $required) {
            throw new RuntimeException(
                'Not enough server disk space. These MP3 stems need about '
    ...
    if ($action === 'file_chunk') {
        ...
        $partPath = $dir . '/stem-' . str_pad((string)$fileIndex, 3, '0', STR_PAD_LEFT) . '.mp3';
        ...
        if (!$input || !$output) {
            ...
            throw new RuntimeException('Could not write the direct MP3 stem.');
        }
        ...
        if ($copied === false) {
            throw new RuntimeException('Could not save the direct MP3 stem.');
        }
        ...
        if ($fileState['next_chunk'] >= $totalChunks) {
            ...
            if ($actual !== (int)$fileState['size']) {
                throw new RuntimeException(
                    'MP3 stem size mismatch for ' . $fileState['name'] . '.'
                );
            }
    ...
    if ($action === 'commit') {
        ...
        try {
            foreach ($state['files'] as $index=>$file) {
                $temp = $dir . '/stem-' . str_pad((string)$index, 3, '0', STR_PAD_LEFT) . '.mp3';
                ...
                $baseName = preg_replace('/\.mp3$/i', '', $baseName) ?: $baseName;
