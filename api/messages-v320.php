...
try{vp3_human_messaging_v370_ensure_schema($pdo);}catch(Throwable $e){vp3_messages_json_v320(['ok'=>false,'error'=>'upgrade_required'],503);}
...
try{
 ...
 if($action==='conversation'){$cid=max(0,(int)($_GET['conversation_id']??0));$after=max(0,(int)($_GET['after']??0));$c=vp3_human_conversation_v370($pdo,$cid);if(!$c||!vp3_human_can_access_v370($pdo,$c,$uid))vp3_messages_json_v320(['ok'=>false,'error'=>'not_found'],404);vp3_messages_json_v320(['ok'=>true,'conversation'=>$c,'request'=>vp3_human_request_v370($pdo,$cid),'messages'=>vp3_human_messages_v370($pdo,$cid,$uid,$after)]);}
