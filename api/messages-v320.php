...
try{
 if($action==='inbox')vp3_messages_json_v320(['ok'=>true,'inbox'=>vp3_social_inbox_v320($pdo,$uid),'settings'=>vp3_social_settings_v320($pdo,$uid)]);
 ...
 if($action==='start'){vp3_messages_require_post_v320();$result=vp3_social_start_direct_v320($pdo,$uid,max(0,(int)($_POST['user_id']??0)),(string)($_POST['message']??''));vp3_messages_json_v320(['ok'=>true]+$result);}
 if($action==='send'){vp3_messages_require_post_v320();vp3_messages_json_v320(['ok'=>true,'message'=>vp3_social_send_message_v320($pdo,max(0,(int)($_POST['conversation_id']??0)),$uid,(string)($_POST['message']??''))]);}
