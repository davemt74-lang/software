...
const STONEFELLOW_DIRECT_BUILD = 'v79';
$responded = false;
...
function direct_v79_native_chain(mixed $value): array
{
    if (!is_array($value)) return [];
    $allowed = ['eq5','compressor','delay','reverb','limiter'];
    $out = [];

    foreach (array_slice($value,0,6) as $plugin) {
        if (!is_array($plugin)) continue;
        $type = trim((string)($plugin['type'] ?? ''));
        if (!in_array($type,$allowed,true)) continue;
        $params = is_array($plugin['params'] ?? null) ? $plugin['params'] : [];
        $clean = [];
        foreach ($params as $key=>$number) {
            if (!is_string($key) || !is_numeric($number)) continue;
            $clean[mb_substr($key,0,40)] = (float)$number;
            if (count($clean) >= 16) break;
        }
        $out[] = [
            'source_name'=>mb_substr(trim((string)($plugin['source_name'] ?? '')),0,160),
            'source_format'=>mb_substr(trim((string)($plugin['source_format'] ?? '')),0,20),
            'source_preset'=>mb_substr(trim((string)($plugin['source_preset'] ?? '')),0,160),
            'source_fxid'=>mb_substr(trim((string)($plugin['source_fxid'] ?? '')),0,80),
            'type'=>$type,
            'enabled'=>($plugin['enabled'] ?? true) !== false,
            'params'=>$clean,
            'mapping'=>in_array((string)($plugin['mapping'] ?? ''),['exact','translated','estimated'],true)
                ? (string)$plugin['mapping'] : 'estimated',
            'confidence'=>max(0.0,min(1.0,(float)($plugin['confidence'] ?? 0.5))),
        ];
    }
...
try {
        ...
        if ($metaJson === '') {
            ...
            foreach (
                ...
                $safeMap[$safeKey] = [
                    ...
                    'plugin_chain'=>direct_v79_native_chain($map['plugin_chain'] ?? []),
            ...
            $rppInfo['master_plugin_chain'] = direct_v79_native_chain($rppInfo['master_plugin_chain'] ?? []);
    ...
    if ($action === 'save_db') {
        ...
        if ($existingProject) {
            ...
            $stmt = $pdo->prepare(
                'INSERT INTO track_projects
                 (track_id,project_name,source_zip_name,
                  ...
                  media_sample_rate,project_start_seconds,master_plugin_chain_json,
            ...
            $stmt->execute([
                ...
                json_encode(direct_v79_native_chain($rppInfo['master_plugin_chain'] ?? []), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),
                $userId,
            ]);
    ...
    if ($action === 'row_add') {
        ...
        $metadataUpdate = $pdo->prepare(
            'UPDATE track_stems
             SET source_track_name=?,channels=?,sample_rate=?,bit_depth=?,
                 rpp_track_guid=?,rpp_volume=?,rpp_pan=?,rpp_fx_summary=?,plugin_chain_json=?
             WHERE id=?'
        );
        $metadataUpdate->execute([
            ...
            $reviewPan,
            $fxSummary,
            json_encode(direct_v79_native_chain($map['plugin_chain'] ?? []), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),
            $rowId,
        ]);
                ...
                $stmt = $pdo->prepare(
                    'UPDATE track_projects
                     SET project_name=?,source_zip_name=?,rpp_file_name=?,rpp_file_path=?,
                         ...
                         project_start_seconds=?,master_plugin_chain_json=?,imported_by_user_id=?,imported_at=NOW()
                     WHERE id=?'
                );
