...
$source=agent_event_name_v1920($_GET['source']??'',80);
...
try{
    ...
    if($e->getMessage()==='This event is already being dispatched.'){http_response_code(202);echo json_encode(['ok'=>true,'accepted'=>true,'duplicate'=>true,'processing_status'=>'processing','build'=>VP3_AGENT_EVENT_INFRA_V1920],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);exit;}
...
}catch(Throwable $e){http_response_code(400);echo json_encode(['ok'=>false,'error'=>'Webhook request rejected.'],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);}
