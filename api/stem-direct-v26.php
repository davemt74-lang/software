...
const STONEFELLOW_DIRECT_BUILD = 'v26';
$responded = false;
...
try {
    ...
    if ($action === 'commit_prepare') {
        ...
        // Remove stale inactive rows left by an interrupted prior attempt for
        // this track. Active/live stems are untouched until commit_finish.
        $pdo->prepare(
            'DELETE FROM track_stems WHERE track_id=? AND is_active=0'
        )->execute([$trackId]);
        ...
        if (
            ...
            if (
                ...
                (string)$existingProject['rpp_file_path'] !== $projectPath
            ) {
                $oldPaths[] = (string)$existingProject['rpp_file_path'];
            }
        }
        ...
        foreach ($oldStems as $oldStem) {
            if (!empty($oldStem['file_path'])) {
                $oldPaths[] = (string)$oldStem['file_path'];
            }
        }
        ...
        $state['commit'] = [
            ...
            'had_existing_project'=>(bool)$existingProject,
        ...
        direct_stem_log(
            $requestId,
            'Commit prepared track=' . $trackId
            . ' stems=' . count($state['files'])
            . ' project_id=' . $projectId
        );
    ...
    if ($action === 'commit_step') {
        ...
        if (!in_array($stemExtension, ['mp3','wav'], true)) {
            throw new RuntimeException(
                'Unsupported stem type during commit: '
                . (string)$file['name']
            );
        }
        ...
        if (!is_file($temp)) {
            throw new RuntimeException(
                'Uploaded stem is missing before commit: '
                . (string)$file['name']
            );
        }
        ...
        $final = rtrim(
            (string)$commit['stem_dir'],
            DIRECTORY_SEPARATOR
        ) . DIRECTORY_SEPARATOR . $savedName;
        ...
        $relative = '/uploads/stems/track-' . $trackId
            . '/' . (string)$commit['import_token']
            . '/' . $savedName;

        $map = stem_match_rpp_file(
            $commit['rpp_info']['file_map'] ?? [],
            (string)$file['name']
        );
        ...
        $offset = max(
            0.0,
            $position - (float)($commit['project_start'] ?? 0.0)
        );
        ...
        // Stage one row per request. It remains invisible to the Stem Studio
        // until commit_finish flips the staged set active.
        $insert = $pdo->prepare(
        ...
        try {
            $insert->execute([
                $trackId,
                (int)$commit['project_id'],
                mb_substr($stemName, 0, 190),
                mb_substr($role, 0, 80),
                ...
                round((float)($map['volume'] ?? 1.0), 6),
                round((float)($map['pan'] ?? 0.0), 6),
                mb_substr($fxSummary, 0, 1000),
                $index + 1,
            ]);
        } catch (Throwable $e) {
            @rename($final, $temp);
        ...
        $commit['new_paths'][] = $relative;
        $commit['max_end'] = max(
            (float)($commit['max_end'] ?? 0),
            $offset + $duration
        );
    ...
    if ($action === 'commit_finish') {
        ...
        if ((int)($commit['next_index'] ?? 0) !== $total) {
            throw new RuntimeException(
                'Not all stems have been staged yet.'
            );
        }

        $rppInfo = $commit['rpp_info'] ?? [];
        $projectId = (int)$commit['project_id'];

        $pdo->beginTransaction();

        try {
            if (!empty($commit['had_existing_project'])) {
                $stmt = $pdo->prepare(
                ...
                $stmt->execute([
                    (string)($rppInfo['project_name'] ?? $track['title']),
                    'Browser ZIP stems',
                    (string)($commit['project_file_name'] ?? ''),
                    (string)($commit['project_path'] ?? ''),
                    $rppInfo['tempo_bpm'] ?? null,
                    (string)($rppInfo['time_signature'] ?? ''),
                    $rppInfo['project_sample_rate'] ?? null,
                    null,
                    (float)($commit['project_start'] ?? 0),
                    $userId,
                    $projectId,
                ]);
            }
        ...
        foreach (($commit['old_paths'] ?? []) as $oldPath) {
        ...
        if (is_array($commit)) {
            try {
                $pdo->prepare(
                    ...
                    $trackId,
                    (int)($commit['project_id'] ?? 0),
                ]);

                if (!empty($commit['created_project'])) {
            ...
            foreach (($commit['new_paths'] ?? []) as $path) {
