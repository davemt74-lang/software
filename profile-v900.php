...
if($commerceReturn==='return'){
    $notice=$_SESSION['profile_commerce_return_notices'][(string)(int)$profile['user_id']]??null;
    unset($_SESSION['profile_commerce_return_notices'][(string)(int)$profile['user_id']]);
    if(is_array($notice)&&(int)($notice['expires_at']??0)>=time()){
        $kind=(string)($notice['kind']??'pending');$headline=(string)($notice['headline']??'Payment return received.');$message=(string)($notice['message']??'VP3 is verifying the provider result.');
        $commerceHtml.='<div class="profile-commerce-return-v900'.($kind==='verified'?'':' pending').'" role="status"><strong>'.e($headline).'</strong><span>'.e($message).'</span></div>';
    }
